...
type Cuenta = { id: number; banco?: string | null; numero_cuenta?: string | null; nombre_banco?: string | null };
...
type PagoRow = {
  ...
  unidades?: { codigo?: string | null; propietario_nombre?: string | null } | null;
};
...
export default function PruebasMotorPagosPage() {
  ...
  const [condominioId, setCondominioId] = useState("15");
  const [unidadId, setUnidadId] = useState("");
  const [cuentaId, setCuentaId] = useState("6");
  ...
  async function cargarCatalogos() {
    const { data } = await supabase.from("condominios").select("id,nombre,descripcion").order("id");
  ...
  async function cargarUnidadesYCuentas() {
    ...
    const [{ data: und }, { data: ctas }] = await Promise.all([
      supabase
        .from("unidades")
      ...
      supabase
        ...
        .select("id,banco,numero_cuenta,nombre_banco")
        .eq("condominio_id", cid)
        .order("id"),
    ]);

    setUnidades((und || []) as Unidad[]);
    setCuentas((ctas || []) as Cuenta[]);
    if (!unidadId && und && und.length > 0) setUnidadId(String(und[0].id));
    if (!cuentaId && ctas && ctas.length > 0) setCuentaId(String(ctas[0].id));
  }
  ...
  async function cargarDatos() {
    ...
    try {
      const [{ data: ana, error: eAna }, { data: pgs, error: ePagos }, { data: cgs, error: eCargos }, { data: bmov, error: eBanco }, { data: cls, error: eCierres }, { data: lgs, error: eLogs }] = await Promise.all([
        ...
        supabase
          .from("pagos")
          .select("id,unidad_id,fecha_pago,periodo,monto,descripcion,referencia,cuenta_bancaria_id,unidades(codigo,propietario_nombre)")
          .eq("condominio_id", cid)
        ...
        supabase
          ...
          .select("id,unidad_id,periodo,monto,monto_pagado,balance,estado,unidades(codigo)")
          .eq("condominio_id", cid)
          .gte("periodo", periodoDesde)
          .lte("periodo", periodoHasta)
          .order("periodo"),
  ...
  async function ejecutarPruebaPago() {
    ...
    if (!condominioId || !unidadId || !cuentaId || !fechaPago || !monto || Number(monto) <= 0) {
      setMensaje("Completa condominio, unidad, cuenta, fecha y monto válido.");
      log("ERROR: datos incompletos para prueba de pago.");
      setLoading(false);
      return;
    }
      ...
      <div className="mx-auto max-w-7xl space-y-5">
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
            <label className="space-y-1 text-sm"><span className="font-semibold text-slate-700">Condominio</span><select value={condominioId} onChange={(e) => setCondominioId(e.target.value)} className="w-full rounded-xl border border-slate-300 px-3 py-2"><option value="">Seleccione</option>{condominios.map(c => <option key={c.id} value={c.id}>{c.nombre || c.descripcion || `Condominio ${c.id}`}</option>)}</select></label>
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-5">
            <label className="space-y-1 text-sm"><span className="font-semibold text-slate-700">Cuenta bancaria</span><select value={cuentaId} onChange={(e) => setCuentaId(e.target.value)} className="w-full rounded-xl border border-slate-300 px-3 py-2">{cuentas.map(c => <option key={c.id} value={c.id}>{c.banco || c.nombre_banco || "Banco"} {c.numero_cuenta || ""}</option>)}</select></label>
...
function TablaPagos({ rows }: { rows: PagoRow[] }) {
  if (!rows.length) return <Empty />;
  return <TableWrap><thead><tr><Th>ID</Th><Th>Unidad</Th><Th>Fecha</Th><Th>Período</Th><Th>Monto</Th><Th>Referencia</Th><Th>Descripción</Th></tr></thead><tbody className="divide-y divide-slate-100 bg-white">{rows.map(r => <tr key={r.id}><Td>{r.id}</Td><Td>{r.unidades?.codigo || r.unidad_id}</Td><Td>{r.fecha_pago}</Td><Td>{r.periodo || "-"}</Td><Td>{dinero(r.monto)}</Td><Td>{r.referencia || "-"}</Td><Td>{r.descripcion || "-"}</Td></tr>)}</tbody></TableWrap>;
}
function TablaCargos({ rows }: { rows: CargoRow[] }) {
  if (!rows.length) return <Empty />;
  return <TableWrap><thead><tr><Th>ID</Th><Th>Unidad</Th><Th>Período</Th><Th>Monto</Th><Th>Pagado</Th><Th>Balance</Th><Th>Estado</Th></tr></thead><tbody className="divide-y divide-slate-100 bg-white">{rows.map(r => <tr key={r.id}><Td>{r.id}</Td><Td>{r.unidades?.codigo || r.unidad_id}</Td><Td>{r.periodo}</Td><Td>{dinero(r.monto)}</Td><Td>{dinero(r.monto_pagado)}</Td><Td>{dinero(r.balance)}</Td><Td>{r.estado}</Td></tr>)}</tbody></TableWrap>;
}
