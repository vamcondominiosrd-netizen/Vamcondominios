...
type CambioUnidadForm = {
  pago_id: number;
  unidad_actual_id: number;
  unidad_actual_codigo: string;
...
type PagoPrevioForm = {
  ...
  referencia: string;
  descripcion: string;
  periodos: PeriodoPagoPrevioForm[];
  motivo: string;
};
...
export default function AdminPagosPage() {
  ...
  const [pagoIdFiltro, setPagoIdFiltro] = useState("");
  ...
  const [cambioUnidad, setCambioUnidad] = useState<CambioUnidadForm | null>(null);
  const [cambiandoUnidad, setCambiandoUnidad] = useState(false);
  const [pagoPrevio, setPagoPrevio] = useState<PagoPrevioForm | null>(null);
  const [registrandoPagoPrevio, setRegistrandoPagoPrevio] = useState(false);
  const [pagoPrevioError, setPagoPrevioError] = useState("");
  ...
  async function cargarPagos() {
    ...
    // Si se indica un ID de pago, la búsqueda se hace por ese ID dentro
    // del condominio y no se restringe por unidad. Esto permite localizar
    // pagos que precisamente estén asignados al apartamento equivocado.
    if (pagoIdFiltro.trim()) {
      const pagoId = Number(pagoIdFiltro.trim());

      if (!Number.isInteger(pagoId) || pagoId <= 0) {
        setError("El ID de pago debe ser un número entero mayor que cero.");
        setLoading(false);
        return;
      }
  ...
  function abrirCambioUnidad(p: Pago) {
    if (!p.unidad_id) {
      setError("El pago no tiene una unidad actual asociada.");
      return;
    }

    const unidadActual = unidadMap[p.unidad_id];

    setCambioUnidad({
      pago_id: p.id,
      unidad_actual_id: p.unidad_id,
      unidad_actual_codigo: unidadActual?.codigo || `Unidad ${p.unidad_id}`,
      unidad_nueva_id: "",
      fecha_pago: safeDate(p.fecha_pago),
      monto: Number(p.monto || 0),
      referencia: p.referencia || "",
  ...
  function cerrarCambioUnidad() {
    if (cambiandoUnidad) return;
    setCambioUnidad(null);
  }

  async function aplicarCambioUnidad() {
    if (!cambioUnidad || !condominioId) return;

    const unidadNuevaId = Number(cambioUnidad.unidad_nueva_id);

    if (!unidadNuevaId) {
      alert("Debe seleccionar la unidad correcta.");
      return;
    }

    if (unidadNuevaId === cambioUnidad.unidad_actual_id) {
      alert("La unidad nueva debe ser diferente de la unidad actual.");
      return;
    }

    const unidadNueva = unidadMap[unidadNuevaId];
    if (!unidadNueva) {
      alert("La unidad seleccionada no pertenece al condominio actual.");
      return;
    }

    if (cambioUnidad.motivo.trim().length < 8) {
      alert("Debe indicar un motivo de corrección de al menos 8 caracteres.");
      return;
    }
    ...
    const confirmar = window.confirm(
      [
        `CAMBIAR UNIDAD DEL PAGO #${cambioUnidad.pago_id}`,
        `Fecha: ${cambioUnidad.fecha_pago}`,
        `Monto: ${money(cambioUnidad.monto)}`,
        `Referencia: ${cambioUnidad.referencia || "-"}`,
        "",
        `Unidad actual: ${cambioUnidad.unidad_actual_codigo}`,
        `Unidad correcta: ${unidadNueva.codigo || `Unidad ${unidadNuevaId}`}`,
        "",
        "La corrección actualizará el pago y su identificación asociada. Si existen aplicaciones del pago, la función segura las reversará antes de moverlo. El movimiento bancario original no se modifica ni se duplica.",
        "",
        "Después del cambio, revise/reconcilie los cargos de la unidad destino para confirmar la distribución mensual.",
        "",
        "¿Desea continuar?",
      ].join("\n"),
    );
    ...
    setCambiandoUnidad(true);
    setError("");
    setMensaje("");

    try {
      const { data, error: rpcError } = await supabase.rpc(
        "admin_cambiar_unidad_pago_seguro",
        {
          p_pago_id: cambioUnidad.pago_id,
          p_unidad_nueva_id: unidadNuevaId,
          p_motivo: cambioUnidad.motivo.trim(),
        },
      );
      ...
      const resultado = data as any;
      const unidadActualCodigo = cambioUnidad.unidad_actual_codigo;
      const unidadNuevaCodigo = unidadNueva.codigo || `Unidad ${unidadNuevaId}`;

      setCambioUnidad(null);
      await cargarPagos();
      setMensaje(
        resultado?.mensaje ||
          `Pago #${cambioUnidad.pago_id} movido de ${unidadActualCodigo} a ${unidadNuevaCodigo}. Reconcile los cargos de ambas unidades si presentan diferencias.`,
      );
    } catch (err: any) {
      setError(
        err?.message ||
          "No se pudo cambiar la unidad del pago. No se realizaron cambios parciales desde esta pantalla.",
      );
    } finally {
      setCambiandoUnidad(false);
    }
  }

  function abrirPagoPrevioApertura() {
    const unidadInicial = unidadFiltro || "";

    setPagoPrevio({
      unidad_id: unidadInicial,
      fecha_pago: "2025-12-31",
      monto: "",
      referencia: "",
      descripcion: "Pago previo al inicio de VAM",
      periodos: [
        { periodo: "2025-12", monto: "" },
        { periodo: "2026-01", monto: "" },
      ],
      motivo: "",
    });
    setError("");
  ...
  function cerrarPagoPrevioApertura() {
    if (registrandoPagoPrevio) return;
    setPagoPrevio(null);
  }

  function actualizarPeriodoPagoPrevio(
    index: number,
    campo: keyof PeriodoPagoPrevioForm,
    valor: string,
  ) {
    setPagoPrevio((actual) => {
      if (!actual) return actual;
      return {
        ...actual,
        periodos: actual.periodos.map((item, posicion) =>
  ...
  function agregarPeriodoPagoPrevio() {
    setPagoPrevio((actual) =>
      actual
        ? {
            ...actual,
            periodos: [...actual.periodos, { periodo: "", monto: "" }],
          }
        : actual,
    );
  }

  function quitarPeriodoPagoPrevio(index: number) {
    setPagoPrevio((actual) => {
      if (!actual || actual.periodos.length <= 1) return actual;
      return {
        ...actual,
  ...
  async function registrarPagoPrevioApertura() {
    if (!pagoPrevio) return;

    if (!condominioId) {
      const mensajeError =
        "No se pudo identificar el condominio activo. Cierre esta ventana, vuelva a abrir Pagos Arreglos e inténtelo nuevamente.";
      setPagoPrevioError(mensajeError);
      setError(mensajeError);
      window.alert(mensajeError);
      return;
    }

    const unidadId = Number(pagoPrevio.unidad_id);
    const montoTotal = Number(pagoPrevio.monto || 0);

    if (!unidadId || !unidadMap[unidadId]) {
      alert("Debe seleccionar la unidad/propietario correcto.");
      return;
    }

    if (!pagoPrevio.fecha_pago) {
      alert("Debe indicar la fecha real en que se realizó el pago previo.");
      return;
    }

    if (pagoPrevio.fecha_pago >= "2026-01-01") {
      alert(
        "Esta opción es para pagos realizados antes del inicio operativo 2026. La fecha debe ser anterior al 01/01/2026.",
      );
      return;
    }

    if (!montoTotal || montoTotal <= 0) {
      alert("El monto total del pago debe ser mayor que cero.");
      return;
    }

    const detalles = pagoPrevio.periodos
      .map((item) => ({
        periodo: item.periodo.trim(),
        monto: Number(item.monto || 0),
      }))
    ...
    if (detalles.length === 0) {
      alert("Debe indicar al menos un período pagado.");
      return;
    }
    ...
    if (
      detalles.some(
        (item) =>
          !/^\d{4}-(0[1-9]|1[0-2])$/.test(item.periodo) || item.monto <= 0,
      )
    ) {
      alert("Revise los períodos y sus montos.");
      return;
    }

    const periodosUnicos = new Set(detalles.map((item) => item.periodo));
    if (periodosUnicos.size !== detalles.length) {
      alert("No puede repetir un mismo período.");
      return;
    }

    const totalDistribuido = detalles.reduce(
      (sum, item) => sum + item.monto,
      0,
    );

    if (Math.round(totalDistribuido * 100) !== Math.round(montoTotal * 100)) {
      alert(
        `La distribución suma ${money(totalDistribuido)} y debe coincidir con el pago ${money(montoTotal)}.`,
      );
      return;
    }

    const tienePeriodoActual = detalles.some((item) => item.periodo >= "2026-01");
    if (!tienePeriodoActual) {
      alert(
        "Este pago solo contiene períodos anteriores a 2026. Para ese caso utilice la opción de pago histórico; Pago previo/apertura se usa cuando al menos una parte afecta cargos desde 2026.",
      );
      return;
    }

    if (pagoPrevio.motivo.trim().length < 8) {
      alert("Debe indicar un motivo de registro de al menos 8 caracteres.");
      return;
    }

    const unidad = unidadMap[unidadId];
    const detalleTexto = detalles
      .map((item) => `${item.periodo}: ${money(item.monto)}`)
      .join("\n");
    ...
    const confirmar = window.confirm(
      [
        "REGISTRAR PAGO PREVIO / SALDO DE APERTURA",
        `Unidad: ${unidad?.codigo || unidadId}`,
        `Fecha real del pago: ${pagoPrevio.fecha_pago}`,
        `Monto total reconocido: ${money(montoTotal)}`,
        "",
        "Distribución:",
        detalleTexto,
        "",
        "IMPORTANTE: este registro NO creará ni modificará banco_movimientos, archivo_banco ni el Control Bancario 2026.",
        "Los períodos de 2026 se aplicarán a sus cargos y los períodos anteriores quedarán visibles en el expediente como parte del saldo de apertura.",
        "",
        "¿Desea continuar?",
      ].join("\n"),
    );
    ...
    try {
      const { data, error: rpcError } = await supabase.rpc(
        "admin_registrar_pago_previo_apertura",
        {
          p_condominio_id: condominioId,
          p_unidad_id: unidadId,
          p_fecha_pago: pagoPrevio.fecha_pago,
          p_monto_total: montoTotal,
          p_periodos: detalles,
          p_referencia: pagoPrevio.referencia.trim() || null,
          p_descripcion:
            pagoPrevio.descripcion.trim() || "Pago previo al inicio de VAM",
          p_motivo: pagoPrevio.motivo.trim(),
        },
      );
      ...
      setMensaje(
        resultado?.mensaje ||
          `Pago previo de ${money(montoTotal)} registrado correctamente para ${unidad?.codigo || unidadId}, sin afectar Control Bancario.`,
      );
    } catch (err: any) {
      const mensajeError =
        err?.message ||
        "No se pudo registrar el pago previo. No se realizaron cambios parciales desde esta pantalla.";

      console.error("Error registrando pago previo/apertura:", err);
      setPagoPrevioError(mensajeError);
      setError(mensajeError);
      window.alert(`No se pudo registrar el pago previo:\n\n${mensajeError}`);
    } finally {
      setRegistrandoPagoPrevio(false);
    }
  }
        ...
        <div className="rounded-2xl bg-white p-6 shadow-sm border border-slate-200">
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex flex-col gap-2 sm:flex-row">
              <button
                onClick={abrirPagoPrevioApertura}
                disabled={loading || saving || registrandoPagoPrevio}
                className="rounded-xl border border-indigo-300 bg-indigo-50 px-4 py-2 text-sm font-black text-indigo-800 hover:bg-indigo-100 disabled:opacity-50"
                title="Registra pagos realizados antes del inicio de VAM que deben verse en el expediente y pueden cubrir cargos de 2026, sin afectar Control Bancario"
              >
                + Pago previo / apertura
              </button>
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-7">
            <div>
              ...
              <input
                ...
                step="1"
                value={pagoIdFiltro}
                onChange={(e) => setPagoIdFiltro(e.target.value)}
                placeholder="Ej. 143"
                      ...
                      <td className="px-4 py-3 text-right">
                        <div className="flex justify-end gap-2">
                          ...
                          <button
                            ...
                            className="rounded-lg border border-fuchsia-300 bg-fuchsia-50 px-3 py-1.5 text-xs font-semibold text-fuchsia-800 hover:bg-fuchsia-100 disabled:opacity-50"
                            title="Corrige el apartamento/unidad asociado a un pago sin alterar el depósito bancario original"
                          >
                            Cambiar unidad
                          </button>
                          <button
      ...
      {pagoPrevio && (
        <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/50 p-4">
          <div className="max-h-[94vh] w-full max-w-4xl overflow-y-auto rounded-2xl bg-white p-6 shadow-2xl">
            <div className="flex flex-col gap-4 border-b border-indigo-100 pb-4 md:flex-row md:items-start md:justify-between">
              <div>
                <p className="text-xs font-black uppercase tracking-wide text-indigo-700">
                  Saldo de apertura / expediente del propietario
                </p>
                <h2 className="mt-1 text-2xl font-black text-slate-900">
                  Registrar pago previo al inicio de VAM
                </h2>
                <p className="mt-1 text-sm text-slate-600">
                  Registra un pago realizado antes de 2026 sin crear ni modificar movimientos del Control Bancario.
                </p>
              </div>
              <button
                type="button"
                onClick={cerrarPagoPrevioApertura}
                disabled={registrandoPagoPrevio}
            ...
            <div className="mt-5 rounded-2xl border border-indigo-200 bg-indigo-50 p-4 text-sm text-indigo-950">
              <p className="font-black">No afecta Control Bancario</p>
              <p className="mt-1 leading-relaxed">
                Esta operación es para pagos realizados antes de que VAM iniciara el control bancario en 2026. El pago quedará visible en el expediente del propietario. Los períodos desde 2026 podrán aplicarse a sus cargos, pero no se generará ningún ingreso bancario nuevo.
              </p>
            </div>

            {pagoPrevioError && (
              <div className="mt-4 rounded-2xl border border-red-300 bg-red-50 p-4 text-sm text-red-900">
                <p className="font-black">No se pudo guardar el pago previo</p>
                <p className="mt-1 whitespace-pre-wrap break-words">{pagoPrevioError}</p>
                <p className="mt-2 text-xs text-red-700">
                  El mensaje se muestra aquí para que podamos identificar exactamente si el problema viene de la función RPC, una restricción SQL o los datos enviados.
                </p>
              </div>
            )}
            ...
            <div className="mt-5 grid grid-cols-1 gap-4 md:grid-cols-2">
              <label className="text-xs font-semibold uppercase text-slate-500">
                Apartamento / Unidad *
                <select
                  value={pagoPrevio.unidad_id}
                  onChange={(e) =>
                    setPagoPrevio({ ...pagoPrevio, unidad_id: e.target.value })
                  }
                  className="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2 text-sm"
                >
                  ...
                  {unidades.map((u) => (
                    <option key={u.id} value={u.id}>
                      {u.codigo || `Unidad ${u.id}`} {u.propietario_nombre ? `- ${u.propietario_nombre}` : ""}
                    </option>
                  ))}
                </select>
              </label>
              ...
              <label className="text-xs font-semibold uppercase text-slate-500">
                Fecha real del pago previo *
                <input
                  type="date"
                  max="2025-12-31"
                  value={pagoPrevio.fecha_pago}
                  onChange={(e) =>
                    setPagoPrevio({ ...pagoPrevio, fecha_pago: e.target.value })
                  }
                  className="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2 text-sm"
                />
              </label>
              ...
              <label className="text-xs font-semibold uppercase text-slate-500">
                ...
                <input
                  ...
                  min="0.01"
                  step="0.01"
                  value={pagoPrevio.monto}
                  onChange={(e) =>
                    setPagoPrevio({ ...pagoPrevio, monto: e.target.value })
                  }
              ...
              <label className="text-xs font-semibold uppercase text-slate-500">
                Referencia / soporte
                <input
                  value={pagoPrevio.referencia}
                  onChange={(e) =>
                    setPagoPrevio({ ...pagoPrevio, referencia: e.target.value })
                  }
                  placeholder="Ej. Saldo de apertura / recibo anterior"
                  className="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2 text-sm"
                />
              </label>
              ...
              <label className="md:col-span-2 text-xs font-semibold uppercase text-slate-500">
                Descripción
                <input
                  value={pagoPrevio.descripcion}
                  onChange={(e) =>
                    setPagoPrevio({ ...pagoPrevio, descripcion: e.target.value })
                  }
                  className="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2 text-sm"
                />
              </label>
            ...
            <div className="mt-6">
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <h3 className="font-black text-slate-900">Distribución del pago</h3>
                  <p className="text-xs text-slate-500">
                    Incluya los meses históricos y los meses de 2026 que realmente cubre el pago.
                  </p>
                </div>
                <button
                  type="button"
                  onClick={agregarPeriodoPagoPrevio}
                  className="rounded-lg border border-indigo-300 bg-white px-3 py-2 text-xs font-bold text-indigo-800 hover:bg-indigo-50"
                >
                  + Agregar período
                </button>
              </div>
              ...
              <div className="mt-3 space-y-3">
                {pagoPrevio.periodos.map((detalle, index) => (
                  <div
                    key={index}
                    className="grid grid-cols-1 gap-3 rounded-xl border border-slate-200 p-3 md:grid-cols-[1fr_1fr_auto] md:items-end"
                  >
                    <label className="text-xs font-semibold uppercase text-slate-500">
                      Período
                      <input
                        ...
                        onChange={(e) =>
                          actualizarPeriodoPagoPrevio(index, "periodo", e.target.value)
                        }
                        className="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2 text-sm"
                      />
                    </label>
                    ...
                    <label className="text-xs font-semibold uppercase text-slate-500">
                      ...
                      <input
                        ...
                        value={detalle.monto}
                        onChange={(e) =>
                          actualizarPeriodoPagoPrevio(index, "monto", e.target.value)
                        }
                        className="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2 text-sm"
                      />
                    </label>
                    ...
                    <button
                      type="button"
                      onClick={() => quitarPeriodoPagoPrevio(index)}
                      disabled={pagoPrevio.periodos.length <= 1}
              ...
              {(() => {
                const monto = Number(pagoPrevio.monto || 0);
                const distribuido = pagoPrevio.periodos.reduce(
                ...
                return (
                  <div className="mt-4 grid grid-cols-3 overflow-hidden rounded-xl border border-slate-200 text-center text-sm">
                    <div className="border-r bg-slate-50 p-3">
                      ...
                      <p className="mt-1 font-black">{money(monto)}</p>
                    </div>
                    <div className="border-r bg-indigo-50 p-3">
                      <p className="text-xs font-bold uppercase text-slate-500">Distribuido</p>
                      <p className="mt-1 font-black text-indigo-800">{money(distribuido)}</p>
                    </div>
                    <div className="bg-slate-50 p-3">
                      <p className="text-xs font-bold uppercase text-slate-500">Diferencia</p>
                      <p className={`mt-1 font-black ${Math.abs(diferencia) < 0.005 ? "text-emerald-700" : "text-red-700"}`}>
                        {money(diferencia)}
                      </p>
                    </div>
                  </div>
            ...
            <div className="mt-5">
              <label className="text-xs font-semibold uppercase text-slate-500">
                Motivo / soporte de apertura *
              </label>
              <textarea
                value={pagoPrevio.motivo}
                onChange={(e) =>
                  setPagoPrevio({ ...pagoPrevio, motivo: e.target.value })
                }
                rows={3}
                placeholder="Ej.: Pago realizado en diciembre de 2025 por RD$9,000, correspondiente a diciembre 2025 y enero 2026, incorporado al expediente al iniciar VAM."
                className="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2 text-sm"
              />
            </div>

            <div className="mt-6 rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs text-amber-900">
              <strong>Control:</strong> esta pantalla no permite registrar el pago previo si la fecha es 2026 o posterior. Para pagos bancarios normales de 2026 utilice el flujo regular de pagos.
            </div>
            ...
            <div className="mt-6 flex flex-col gap-3 md:flex-row md:justify-end">
              ...
              <button
                type="button"
                onClick={registrarPagoPrevioApertura}
                disabled={registrandoPagoPrevio}
                className="rounded-xl bg-indigo-700 px-5 py-2.5 text-sm font-black text-white hover:bg-indigo-800 disabled:opacity-50"
              >
                {registrandoPagoPrevio ? "Guardando..." : "Guardar pago previo"}
              </button>
            </div>
          </div>
        </div>
      ...
      {cambioUnidad && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/50 p-4">
          <div className="max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-2xl bg-white p-6 shadow-2xl">
            <div className="flex flex-col gap-4 border-b border-fuchsia-100 pb-4 md:flex-row md:items-start md:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-600">
                  Fecha {cambioUnidad.fecha_pago} · {money(cambioUnidad.monto)}
                </p>
              </div>

              <button
                onClick={cerrarCambioUnidad}
            ...
            <div className="mt-5 rounded-2xl border border-fuchsia-200 bg-fuchsia-50 p-4 text-sm text-fuchsia-950">
              <p className="font-black">¿Cuándo usar esta opción?</p>
              <p className="mt-1 leading-relaxed">
                Cuando un depósito real fue identificado al apartamento equivocado. Esta corrección mueve el pago a la unidad correcta y conserva intacto el movimiento bancario original para no duplicar ni alterar el cuadre del banco.
              </p>
            </div>
            ...
            <div className="mt-5 grid grid-cols-1 gap-4 md:grid-cols-2">
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                ...
                <dl className="mt-3 space-y-2 text-sm">
                  <div className="flex justify-between gap-3">
                    <dt className="text-slate-500">Unidad actual</dt>
                    <dd className="font-black text-red-700">
                      {cambioUnidad.unidad_actual_codigo}
                    </dd>
                  </div>
                  <div className="flex justify-between gap-3">
                    <dt className="text-slate-500">Monto</dt>
                    <dd className="font-bold">{money(cambioUnidad.monto)}</dd>
                  </div>
                  <div className="flex justify-between gap-3">
                    <dt className="text-slate-500">Referencia</dt>
                    <dd className="font-semibold">{cambioUnidad.referencia || "-"}</dd>
                  </div>
                </dl>
              </div>
              ...
              <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4">
                ...
                <select
                  value={cambioUnidad.unidad_nueva_id}
                  onChange={(e) =>
                    setCambioUnidad({
                      ...cambioUnidad,
                      unidad_nueva_id: e.target.value,
                    })
                  }
                  className="mt-2 w-full rounded-xl border border-emerald-300 bg-white px-3 py-2 text-sm font-semibold text-slate-800"
                >
                  <option value="">Seleccione...</option>
                  {unidades
                    .filter((u) => u.id !== cambioUnidad.unidad_actual_id)
                    .map((u) => (
                      <option key={u.id} value={u.id}>
                        {u.codigo || `Unidad ${u.id}`}
                        {u.propietario_nombre ? ` · ${u.propietario_nombre}` : ""}
                      </option>
                    ))}
                </select>

                {cambioUnidad.unidad_nueva_id && (
                  <p className="mt-3 text-xs font-bold text-emerald-800">
                    Destino: {unidadMap[Number(cambioUnidad.unidad_nueva_id)]?.codigo || "-"}
                  </p>
                )}
              </div>
            </div>

            {cambioUnidad.descripcion && (
            ...
            <div className="mt-5">
              ...
              <textarea
                value={cambioUnidad.motivo}
                onChange={(e) =>
                  setCambioUnidad({ ...cambioUnidad, motivo: e.target.value })
                }
                rows={3}
                placeholder="Ej.: El depósito fue identificado inicialmente a otra unidad; corresponde realmente a L9-L1."
                className="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2 text-sm"
              />
            </div>

            <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs leading-relaxed text-amber-900">
              El cambio de unidad no altera la fecha ni el monto del depósito bancario. Después de corregir la identificación, utilice <strong>Reconciliar cargos</strong> para validar la distribución mensual de la unidad destino y, si corresponde, de la unidad anterior.
            </div>
            ...
            <div className="mt-6 flex flex-col gap-3 md:flex-row md:justify-end">
              ...
              <button
                onClick={aplicarCambioUnidad}
                disabled={cambiandoUnidad || !cambioUnidad.unidad_nueva_id}
                className="rounded-xl bg-fuchsia-700 px-5 py-2.5 text-sm font-black text-white hover:bg-fuchsia-800 disabled:opacity-50"
              >
                {cambiandoUnidad ? "Corrigiendo..." : "Cambiar a unidad correcta"}
              </button>
            </div>
          </div>
        </div>
