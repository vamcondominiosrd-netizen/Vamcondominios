  ...
  async function calcularMontoPendienteCajaChica() {
    try {
      ...
      const [gastosResultado, fondosResultado] = await Promise.all([
        ...
        supabase
          .from("caja_chica_fondos")
          .select("id, tipo, monto, estado, movimiento_banco_id")
      ...
      const fondosDesembolsados = fondos.filter(
        (f) =>
          !normalizarTexto(f.estado).includes("anulad") &&
          Boolean(f.movimiento_banco_id),
      );

      const fondoInicial = fondosDesembolsados.find(
      ...
      if (fondoFijo <= 0) {
        setMensajeCajaChica(
          "No se encontró un Fondo Inicial de Caja Chica desembolsado. Revise la configuración antes de solicitar una reposición.",
        );
        setMonto("");
        return;
      }
