  ...
  async function cargarSolicitudes(
    ...
    if (idsSolicitudes.length > 0) {
      const { data: datosCheque, error: chequeMetaError } = await supabase
        ...
        .select(
          "id, estado_cheque, numero_cheque_emitido, fecha_emision_cheque, cuenta_bancaria_cheque_id, beneficiario_cheque, comentario_cheque, cheque_impreso_at, cheque_impreso_por, cheque_impresiones",
        )
        .in("id", idsSolicitudes);

      if (chequeMetaError) {
        console.warn(
          "No fue posible cargar el control de cheques impresos:",
        ...
        for (const item of datosCheque || []) {
          chequeMap.set(Number(item.id), item);
        }
        ...
        solicitudesConCheque = solicitudesCompletas.map((solicitud) => {
          ...
          return cheque
            ? {
                ...solicitud,
                estado_cheque: cheque.estado_cheque,
                numero_cheque_emitido: cheque.numero_cheque_emitido,
                fecha_emision_cheque: cheque.fecha_emision_cheque,
                cuenta_bancaria_cheque_id:
                  cheque.cuenta_bancaria_cheque_id,
                beneficiario_cheque: cheque.beneficiario_cheque,
                comentario_cheque: cheque.comentario_cheque,
                cheque_impreso_at: cheque.cheque_impreso_at,
                cheque_impreso_por: cheque.cheque_impreso_por,
                cheque_impresiones: cheque.cheque_impresiones,
              }
            : solicitud;
        });
      }
