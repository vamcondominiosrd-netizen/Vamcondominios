...
type BotonImprimirChequePagoProps = {
  solicitudId: number;
...
function BotonImprimirChequePago({
  solicitudId,
  numeroSolicitud,
  beneficiario,
  monto,
  ...
  cuenta,
  condominio = "",
}: BotonImprimirChequePagoProps) {
  function abrirImpresion() {
    if (!normalizar(metodoPago).includes("cheque")) {
      alert(
        "La impresión de cheque solo está disponible cuando el método de pago es Cheque.",
      );
      return;
    }
    ...
    window.open("/administracion/cheques/imprimir", "_blank", "noopener,noreferrer");
  }
...
export default function SolicitudesPagoPage() {
  ...
  async function cargarSolicitudes(
    ...
    const solicitudesCajaChica = solicitudesBase.filter(esSolicitudCajaChica);
    ...
    const solicitudesFinales = solicitudesBase.map((solicitud) => {
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  ...
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                      <p className="font-black text-blue-900">
                        Imprimir cheque Banco Popular
                      </p>
                      <p className="mt-1 text-xs font-semibold text-blue-700">
                        VAM tomará automáticamente el proveedor, monto, fecha y número de cheque.
                      </p>
                    </div>

                    <BotonImprimirChequePago
                      solicitudId={modalPago.solicitud_id}
                      numeroSolicitud={numeroSolicitud(modalPago)}
                      beneficiario={modalPago.proveedor_nombre || ""}
                      monto={Number(modalPago.total_solicitud || 0)}
                      fecha={pagoForm.fecha_pago}
                      numeroCheque={pagoForm.numero_documento}
                      concepto={modalPago.concepto || ""}
                      ...
                      cuenta={
                        cuentas.find(
                          (cuenta) =>
                            String(cuenta.id) ===
                            String(pagoForm.cuenta_bancaria_id),
                        ) || null
                      }
                      condominio={modalPago.condominio || condominioNombre}
                    />
                  </div>
                </div>
              )}
              ...
              <div className="md:col-span-2">
                ...
                <p className="mt-1 text-xs text-slate-500">
                  {esSolicitudCajaChica(modalPago)
                    ? "Este comprobante quedará vinculado a la operación de Caja Chica y a la solicitud."
