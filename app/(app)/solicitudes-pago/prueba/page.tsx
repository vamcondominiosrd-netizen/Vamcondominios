...
export default function SolicitudesPagoPage() {
  ...
  const solicitudesFiltradas = useMemo(() => {
    ...
    return solicitudes.filter((s) => {
      ...
      const cumpleVista = perteneceVistaRapida(s, vistaRapida);
      const cumpleEstado =
        filtroEstado === "" ||
        normalizar(estadoOperativo) === normalizar(filtroEstado);

      const fechaParaFiltro =
        vistaRapida === "PAGADAS"
      ...
      const cumpleMes =
        filtroMes === "" ||
        String(fechaParaFiltro || "").startsWith(filtroMes);
      ...
      const texto = normalizar(
        `${s.solicitud_id} ${s.numero_solicitud || ""} ${
          ...
          s.metodo_pago || ""
        } ${s.cuenta_banco || ""} ${s.estado_gasto || ""} ${
          s.estado_solicitud || ""
        } ${s.estado_operativo || ""} ${s.fecha_pago || ""} ${
          s.ultima_fecha_banco || ""
        }`,
      );
  ...
  const resumenFiltros = useMemo(() => {
    ...
    return {
      pendientes: calcular((s) => !esSolicitudPagada(s)),
      porPagar: calcular((s) => esSolicitudListaParaPagar(s)),
      revision: calcular((s) =>
        esEstadoRevision(s.estado_operativo),
      ),
      pagadas: calcular((s) => esSolicitudPagada(s)),
      todas: calcular(() => true),
    };
  }, [solicitudes]);
  ...
  return (
    <PageContainer>
      ...
      <SectionCard
        title="Filtros"
        subtitle="Filtre por condición, mes, estado o documento sin alterar el funcionamiento del módulo."
      >
        <div className="mb-5 grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-5">
          <FiltroRapidoCard
            activa={vistaRapida === "PENDIENTES"}
            titulo="Pendientes"
            cantidad={resumenFiltros.pendientes.cantidad}
            total={resumenFiltros.pendientes.total}
            tono="amber"
            onClick={() => aplicarVista("PENDIENTES")}
          />
          <FiltroRapidoCard
            activa={vistaRapida === "POR_PAGAR"}
            titulo="Por pagar"
            cantidad={resumenFiltros.porPagar.cantidad}
            total={resumenFiltros.porPagar.total}
            tono="purple"
            onClick={() => aplicarVista("POR_PAGAR")}
          />
          <FiltroRapidoCard
            activa={vistaRapida === "REVISION"}
            titulo="Requieren revisión"
            cantidad={resumenFiltros.revision.cantidad}
            total={resumenFiltros.revision.total}
            tono="red"
            onClick={() => aplicarVista("REVISION")}
          />
          <FiltroRapidoCard
            activa={vistaRapida === "PAGADAS"}
            titulo="Pagadas"
            cantidad={resumenFiltros.pagadas.cantidad}
            total={resumenFiltros.pagadas.total}
            tono="green"
            onClick={() => aplicarVista("PAGADAS")}
          />
          <FiltroRapidoCard
            activa={vistaRapida === "TODAS"}
            titulo="Todas"
            cantidad={resumenFiltros.todas.cantidad}
            total={resumenFiltros.todas.total}
            tono="slate"
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-12">
          ...
          <div className="lg:col-span-2">
            ...
            <p className="mt-1 text-xs text-slate-500">
              En Pagadas usa la fecha real de pago.
            </p>
          </div>
          ...
          <div className="lg:col-span-6">
            ...
            <div className="flex items-center gap-2 rounded-xl border bg-white px-4 py-3">
              ...
              <input
                ...
                placeholder="Número, proveedor, concepto, factura, NCF o estado..."
              />
            </div>
          </div>
...
function FiltroRapidoCard({
  ...
  cantidad: number;
  total: number;
  tono: "amber" | "purple" | "red" | "green" | "slate";
  onClick: () => void;
}) {
  const tonos = {
  ...
  return (
    <button
      ...
      className={`rounded-2xl border p-4 text-left shadow-sm transition hover:-translate-y-0.5 hover:shadow-md ${
        tonos[tono]
      } ${activa ? "ring-2 ring-blue-600 ring-offset-2" : ""}`}
    >
      <div className="text-sm font-black">{titulo}</div>
      <div className="mt-2 text-2xl font-black">{cantidad}</div>
