...
export default function SolicitudesPagoPage() {
  ...
  async function cargarSolicitudes(
    ...
    setSolicitudes(
      (data as SolicitudPago[]) || []
    );
  }

  async function generarGasto(
    s: SolicitudPago
  ) {
    ...
    if (s.gasto_generado_id) {
      ...
      alert(
        "Esta solicitud ya fue convertida en gasto."
      );
    ...
    solicitudes.filter((s) => {
      ...
      const texto =
        `${s.concepto} ${s.detalle} ${
          s.catalogo_proveedores
            ?.nombre_proveedor || ""
        } ${
          s.catalogo_categoria_gastos
            ?.nombre_categoria || ""
        }`.toLowerCase();
  ...
  function exportarExcel() {
    ...
    XLSX.writeFile(
      libro,
      `Solicitudes_${condominioNombre}.xlsx`
    );
  }
