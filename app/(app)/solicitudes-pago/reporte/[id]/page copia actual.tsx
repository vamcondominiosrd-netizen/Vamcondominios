...
export default function ReporteSolicitudPagoPage() {
  ...
  const [detallesFacturas, setDetallesFacturas] = useState<DetalleFactura[]>([]);
  ...
  async function cargarReporte() {
    ...
    const { data: detalleData } = await supabase
      ...
      .select("id, solicitud_pago_id, no_factura, fecha_factura, ncf, monto")
      .eq("solicitud_pago_id", Number(solicitudId))
      .order("id", { ascending: true });

    setDetallesFacturas((detalleData as DetalleFactura[]) || []);
    ...
    if (condominioId) {
      ...
      if (condominioData) setCondominio(condominioData as Condominio);
    ...
    if (solicitudActual.gasto_generado_id) {
      ...
      if (gastoData) setGastoRelacionado(gastoData as GastoRelacionado);
    }
  ...
  const esAgrupada =
    detallesFacturas.length > 1 ||
    solicitud?.no_factura === "VARIAS" ||
    solicitud?.ncf === "VARIOS";
      ...
      <section className="pagina-carta max-w-[8.5in] mx-auto bg-white rounded-2xl shadow border text-[10.5px] text-slate-900">
        <div className="contenido-reporte">
          ...
          <div className="grid grid-cols-2 gap-2 mt-3">
            <div className="border rounded-lg p-2">
              ...
              <div className="grid grid-cols-2 gap-x-2 gap-y-1">
                ...
                <p>{esAgrupada ? "VARIAS" : solicitud.no_factura || "-"}</p>

                <p className="font-bold">NCF:</p>
                <p>{esAgrupada ? "VARIOS" : solicitud.ncf || "-"}</p>
          ...
          <div className="border rounded-lg p-2 mt-2">
            ...
            {esAgrupada && detallesFacturas.length > 0 ? (
              <div className="mt-2">
                <p className="font-bold mb-1">Detalle de facturas:</p>

                <table className="w-full text-[9.5px] border-collapse">
                  <thead>
                    <tr className="bg-slate-100">
                      <th className="border p-1 text-left">No. Factura</th>
                      <th className="border p-1 text-left">Fecha</th>
                      <th className="border p-1 text-left">NCF</th>
                      <th className="border p-1 text-right">Monto</th>
                    </tr>
                  </thead>

                  <tbody>
                    {detallesFacturas.map((d) => (
                      <tr key={d.id}>
                        <td className="border p-1 font-bold">
                          {d.no_factura || "-"}
                        </td>
                        <td className="border p-1">
                          {formatoFecha(d.fecha_factura)}
                        </td>
                        <td className="border p-1">{d.ncf || "-"}</td>
                ...
                <div className="mt-2 grid grid-cols-2 gap-2 text-[10px]">
                  <p>
                    <strong>Total facturas:</strong> {detallesFacturas.length}
                  </p>
                  <p className="text-right">
                    <strong>Total general:</strong> RD$ {dinero(solicitud.total)}
                  </p>
                </div>
              </div>
            ) : (
              <div className="mt-1 h-[48px] overflow-hidden">
                ...
                <p className="leading-tight whitespace-pre-line">{detalle}</p>
              </div>
            )}
          </div>
