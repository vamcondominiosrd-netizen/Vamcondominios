...
export default function ReporteSolicitudPagoPage() {
  ...
  const [tesorero, setTesorero] = useState<UsuarioAdmin | null>(null);
  const [presidente, setPresidente] = useState<UsuarioAdmin | null>(null);
  ...
  async function cargarReporte() {
    ...
    if (condominioId) {
      ...
      const { data: firmantesData } = await supabase
        .from("usuarios_admin")
        .select("id, nombre, rol, estado")
        .eq("condominio_id", Number(condominioId))
        .eq("estado", "Activo")
        .in("rol", ["tesorero", "presidente"]);

      const firmantes = (firmantesData || []) as UsuarioAdmin[];

      setTesorero(
        firmantes.find(
          (u) => String(u.rol || "").toLowerCase() === "tesorero"
        ) || null
      );

      setPresidente(
        firmantes.find(
          (u) => String(u.rol || "").toLowerCase() === "presidente"
        ) || null
      );
    }
  ...
  const fechaTesorero =
    solicitud?.fecha_aprobacion_tesorero || solicitud?.aprobado_tesorero_at;

  const fechaPresidente =
    solicitud?.fecha_aprobacion_presidente || solicitud?.aprobado_presidente_at;
      ...
      <section className="print-page max-w-4xl mx-auto bg-white rounded-2xl shadow border p-7 text-[12px] text-slate-900">
        ...
        <div className="grid grid-cols-2 gap-3 mt-4">
          <div className="border rounded-xl p-3">
            ...
            <div className="grid grid-cols-2 gap-x-3 gap-y-1">
              ...
              <p>{solicitud.cuenta_banco || solicitud.banco || "-"}</p>
            </div>
          </div>
          ...
          <div className="border rounded-xl p-3">
            ...
            <div className="mt-3">
              <p className="font-bold">Soporte adjunto:</p>
              <p>{solicitud.soporte_url ? "Sí" : "No"}</p>
            </div>
          </div>
        </div>
        ...
        <div className="border rounded-xl p-3 mt-3">
          ...
          <div className="mt-2 min-h-[54px]">
            ...
            <p className="leading-snug line-clamp-3">{detalle}</p>
          </div>
        </div>

        <div className="border rounded-xl p-3 mt-3">
          <h3 className="font-black uppercase border-b pb-1 mb-3">
          ...
          <div className="grid grid-cols-2 gap-5">
            <div>
              ...
              <div className="mt-2 space-y-1">
                ...
                <p>
                  ...
                  {solicitud.aprobado_tesorero ? "Aprobado" : "Pendiente"}
                </p>

                <p>
                  <strong>Fecha:</strong> {formatoFecha(fechaTesorero)}
                </p>
              </div>
            ...
            <div>
              ...
              <div className="mt-2 space-y-1">
                ...
                <p>
                  <strong>Estado:</strong>{" "}
                  {solicitud.aprobado_presidente ? "Aprobado" : "Pendiente"}
                </p>

                <p>
                  <strong>Fecha:</strong> {formatoFecha(fechaPresidente)}
                </p>
              </div>
        ...
        <div className="border rounded-xl p-3 mt-3">
          ...
          <div className="grid grid-cols-4 gap-x-3 gap-y-2">
            ...
            <p>{solicitud.pagado ? "Sí" : "No"}</p>

            <p className="font-bold">Fecha pago:</p>
            <p>{formatoFecha(solicitud.fecha_pago)}</p>

            <p className="font-bold">No. cheque/ref.:</p>
            <p>{solicitud.numero_cheque || "-"}</p>

            <p className="font-bold">Banco:</p>
            <p>{solicitud.banco || solicitud.cuenta_banco || "-"}</p>
          </div>
