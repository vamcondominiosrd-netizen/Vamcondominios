...
export default function SolicitudesPagoPage() {
  ...
  async function cargarSolicitudes(
    ...
    const solicitudesCajaChica =
      solicitudesCompletas.filter(esSolicitudCajaChica);
    ...
    const solicitudesFinales = solicitudesCompletas.map((solicitud) => {
  ...
  function puedeEmitirChequeSolicitud(s: SolicitudPagoOperativa) {
    if (s.estado_operativo === "Pagada") return false;

    const cajaChicaLista =
      esSolicitudCajaChica(s) && esCajaChicaAprobadaParaPago(s);

    const listaParaPagar =
      s.estado_operativo === "Lista para pagar" || cajaChicaLista;

    return listaParaPagar && solicitudUsaCheque(s);
  }

  function abrirModalEmisionCheque(s: SolicitudPagoOperativa) {
    if (!puedeEmitirChequeSolicitud(s)) {
      alert(
        "Esta solicitud todavía no está lista para emitir cheque o su método de pago no es Cheque.",
      );
      return;
    }
  ...
  function imprimirChequeSolicitud() {
    ...
    const beneficiario = String(modalCheque.proveedor_nombre || "").trim();
          ...
          <div className="w-full max-w-xl rounded-3xl bg-white p-6 shadow-2xl">
            ...
            <div className="mb-5 rounded-2xl border border-blue-100 bg-blue-50 p-4">
              ...
              <div className="mt-1 text-base font-black text-slate-900">
                {modalCheque.proveedor_nombre || "Proveedor no identificado"}
              </div>
