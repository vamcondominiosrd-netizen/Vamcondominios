...
function esEstadoRevision(estado?: string | null) {
  return String(estado || "").startsWith("Revisar");
}
...
function perteneceVistaRapida(
  ...
  const estado = solicitud.estado_operativo || "";

  if (vista === "POR_PAGAR") {
    return estado === "Lista para pagar";
  }

  if (vista === "REVISION") {
    return esEstadoRevision(estado);
  }
...
export default function SolicitudesPagoPage() {
  ...
  const solicitudesFiltradas = useMemo(() => {
    ...
    const resultado = solicitudes.filter((s) => {
      ...
      const cumpleEstado =
        filtroEstado === "" ||
        estadoOperativo === filtroEstado;
      const cumpleMes =
        filtroMes === "" ||
        String(s.fecha_solicitud || "").startsWith(
          filtroMes,
        );
