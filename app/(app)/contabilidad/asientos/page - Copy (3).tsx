...
export default function AsientosPage() {
  ...
  const totales = useMemo(() => {
    const totalDebito = lineas.reduce(
      (acc, linea) => acc + Number(linea.debito || 0),
      0
    );

    const totalCredito = lineas.reduce(
                  ...
                  asientosFiltrados.map((item) => (
                    <tr key={item.id} className="hover:bg-slate-50">
                      ...
                      <td className="px-4 py-3 text-right">
                        {formatoMonto(Number(item.total_debito || 0))}
                      </td>
                      <td className="px-4 py-3 text-right">
                        {formatoMonto(Number(item.total_credito || 0))}
                      </td>
                        ...
                        <tr key={index}>
                          ...
                          <td className="px-3 py-2">
                            <input
                              ...
                              value={linea.debito}
                              onChange={(e) =>
                                actualizarLinea(index, "debito", e.target.value)
                              }
                          ...
                          <td className="px-3 py-2">
                            <input
                              ...
                              value={linea.credito}
                              onChange={(e) =>
                                actualizarLinea(index, "credito", e.target.value)
                              }
                              className="w-32 rounded-xl border border-slate-300 px-3 py-2 text-right text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100"
                            />
                          </td>
