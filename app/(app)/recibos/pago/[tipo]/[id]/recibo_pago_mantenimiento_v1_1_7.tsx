 ...
 * Módulo: Recibo de Pago de Mantenimiento
 * Versión: v1.1.7
 * Fecha: 17/09/2026
 *
 * Cambios v1.1.5:
 * - Corrige "Fecha real pago" para que represente la fecha de liquidación
 *   del cargo: la primera fecha bancaria en que el acumulado aplicado alcanza
 *   el monto total del cargo.
 * - La fecha se calcula por cargo_periodico_id usando
 *   pagos_aplicaciones.monto_aplicado + pagos.fecha_pago.
 * - Si un cargo recibe varios abonos, no se toma simplemente la última fecha:
 *   se toma la fecha exacta en que el cargo queda totalmente saldado.
 * - Mantiene el respaldo histórico por período únicamente cuando no existen
 *   aplicaciones directas suficientes y nunca lo usa para SALDO_INICIAL.
 * - Agrega resumen anual con Total recibido, Aplicado a cargos generados y
 *   Saldo a favor disponible, evitando confundir crédito futuro con pago aplicado.
 *
 * Cambios v1.1.4:
 * - Completa la fecha real de pago de cargos históricos que no tienen relación
 *   directa en pagos_aplicaciones.
 * - Mantiene pagos_aplicaciones -> pagos.fecha_pago como fuente principal.
 * - Como respaldo usa los pagos anuales ya conciliados/resueltos por período
 *   (pagos.periodo, pagos_identificados.periodo o trazabilidad histórica).
 * - El respaldo por período NO se aplica a SALDO_INICIAL para evitar confundirlo
 *   con el cargo ordinario de enero cuando ambos usan periodo 2026-01.
 * - Los cargos pendientes continúan mostrando "-".
 *
 * Cambios v1.1.3:
 * - Agrega la fecha real de pago por cargo en el Estado mensual del recibo.
 * - La fecha se obtiene desde pagos.fecha_pago mediante pagos_aplicaciones.
 * - No usa created_at ni la fecha del cargo como sustituto.
 * - Los cargos pendientes muestran "-" hasta tener un pago realmente aplicado.
 *
 * Cambios v1.1.2:
 * - Corrige el Estado mensual para no duplicar pagos cuando existen dos cargos
 *   con el mismo período (ej. SALDO_INICIAL + mantenimiento de enero).
 * - La columna del resumen anual pasa de "Pago recibido" a "Pago aplicado".
 * - El monto mostrado por fila proviene directamente de monto_pagado del cargo,
 *   que representa lo realmente aplicado a ese cargo.
 * - SALDO_INICIAL se identifica como una fila de apertura histórica y no como
 *   un segundo cargo ordinario de enero.
 * - Ordena el saldo inicial antes del cargo ordinario del mismo mes.
 */

const MODULO_NOMBRE = "Recibo de Pago de Mantenimiento";
const MODULO_VERSION = "v1.1.5";
...
export default function ReciboPagoPage() {
  ...
  const [totalRecibidoAnio, setTotalRecibidoAnio] = useState<number>(0);
  const [cuotaOrdinaria, setCuotaOrdinaria] = useState<number>(0);
  const [fechasPagoPorCargo, setFechasPagoPorCargo] = useState<Record<number, string>>({});
  ...
  async function cargarPagosAnualesReales(
    ...
    /*
     * Total realmente recibido en banco durante el año del recibo.
     * Se suma cada pago una sola vez por su fecha bancaria real, sin importar
     * a cuántos cargos o períodos haya sido distribuido posteriormente.
     */
    setTotalRecibidoAnio(
      pagosBase
        .filter((item) =>
          String(item.fecha_pago || "").startsWith(`${anio}-`)
        )
    ...
    for (const item of pagosBase) {
      ...
      if (periodoIdentificado) {
        const cuotaPeriodoTope = Number(cargosPorPeriodo.get(periodoIdentificado) || 0);

        /*
         * Si el depósito histórico equivale a una sola cuota del período
         * identificado, respeta ese mes de forma directa. Solo los pagos que
         * realmente cubren varias cuotas se distribuyen hacia atrás.
         */
        if (cuotaPeriodoTope > 0 && montoPago <= cuotaPeriodoTope + 0.0001) {
          agregarFila(item, periodoIdentificado, montoPago);
        } else {
  ...
  async function cargarFechasRealesPorCargo(cargosBase: CargoPeriodico[]) {
    const idsCargos = cargosBase
      .map((cargo) => Number(cargo.id || 0))
      .filter((id) => id > 0);
    ...
    const montoCargoPorId = new Map<number, number>();

    for (const cargo of cargosBase) {
      const cargoId = Number(cargo.id || 0);
      if (!cargoId) continue;
      montoCargoPorId.set(cargoId, Number(cargo.monto || 0));
    }

    const { data: aplicacionesData, error: aplicacionesError } = await supabase
      .from("pagos_aplicaciones")
      .select("id, cargo_periodico_id, pago_id, monto_aplicado")
      .in("cargo_periodico_id", idsCargos);

    if (aplicacionesError) {
      console.warn(
        "No se pudieron cargar las aplicaciones para obtener fechas reales de pago:",
    ...
    const aplicaciones = (aplicacionesData || []) as Array<{
      id: number | null;
      cargo_periodico_id: number | null;
      pago_id: number | null;
      monto_aplicado: number | null;
    ...
    const idsPagos = Array.from(
      new Set(
        aplicaciones
          .map((item) => Number(item.pago_id || 0))
          .filter((id) => id > 0)
      )
    );
    ...
    if (pagosError) {
      console.warn(
        "No se pudieron cargar las fechas reales desde pagos:",
        pagosError.message
      );
      setFechasPagoPorCargo({});
      return;
    ...
    const fechaPorPago = new Map<number, string>();

    for (const item of (pagosData || []) as Array<{
      id: number;
      fecha_pago: string | null;
    }>) {
      if (item.fecha_pago) {
        fechaPorPago.set(
          Number(item.id),
          String(item.fecha_pago).slice(0, 10)
        );
      }
    }

    const aplicacionesPorCargo = new Map<
      number,
      Array<{ id: number; fecha: string; monto: number }>
    >();

    for (const aplicacion of aplicaciones) {
      const cargoId = Number(aplicacion.cargo_periodico_id || 0);
      const pagoIdAplicacion = Number(aplicacion.pago_id || 0);
      const fecha = fechaPorPago.get(pagoIdAplicacion) || "";
      const montoAplicado = Number(aplicacion.monto_aplicado || 0);

      if (!cargoId || !fecha || montoAplicado <= 0) continue;

      const lista = aplicacionesPorCargo.get(cargoId) || [];
      lista.push({
        id: Number(aplicacion.id || 0),
        fecha,
        monto: montoAplicado,
      });
      aplicacionesPorCargo.set(cargoId, lista);
    }

    const resultado: Record<number, string> = {};

    for (const [cargoId, lista] of aplicacionesPorCargo.entries()) {
      const montoCargo = Number(montoCargoPorId.get(cargoId) || 0);
      if (montoCargo <= 0) continue;
      ...
      for (const aplicacion of lista) {
        acumulado += Number(aplicacion.monto || 0);

        /*
         * Fecha real de liquidación: primera fecha bancaria en la que el
         * acumulado aplicado alcanza el monto total del cargo.
         */
        if (acumulado + 0.0001 >= montoCargo) {
          resultado[cargoId] = aplicacion.fecha;
          break;
        }
      }
    }

    setFechasPagoPorCargo(resultado);
  }
  ...
  async function cargarCargosPeriodicos(
    ...
    if (data && data.length > 0) {
      const cargosCargados = (data || []) as CargoPeriodico[];
      setCargos(cargosCargados);
      await cargarFechasRealesPorCargo(cargosCargados);
      return;
    }
    ...
    if (condominioId && apartamento) {
      ...
      if (unidadEncontrada) {
        ...
        if (!errorRespaldo && dataRespaldo && dataRespaldo.length > 0) {
          const cargosRespaldo = (dataRespaldo || []) as CargoPeriodico[];
          setCargos(cargosRespaldo);
          await cargarFechasRealesPorCargo(cargosRespaldo);
          return;
        }
      }
    }
  ...
  /*
   * Respaldo de fecha real por período.
   *
   * Fuente principal: fecha de liquidación calculada por cargo usando
   * pagos_aplicaciones.monto_aplicado -> pagos.fecha_pago
   * (fechasPagoPorCargo).
   *
   * Para registros históricos/migrados que tienen el cargo marcado PAGADO pero
   * todavía no poseen pagos_aplicaciones, pagosAnualesReales ya resuelve el
   * período real usando la trazabilidad disponible del pago. Esta tabla de
   * respaldo permite mostrar la fecha bancaria real sin usar created_at ni
   * inventar una fecha desde el cargo.
   */
  const fechasPagoPorPeriodo = useMemo(() => {
    const mapa = new Map<string, string>();

    for (const item of pagosAnualesReales) {
      const periodo = periodoValido(item.periodo);
      const fecha = String(item.fecha_pago || "").slice(0, 10);

      if (!periodo || !fecha) continue;

      const actual = mapa.get(periodo) || "";

      // Respaldo histórico: conserva la fecha bancaria más reciente resuelta
      // para el período cuando no existe trazabilidad directa suficiente por cargo.
      if (!actual || fecha > actual) {
        mapa.set(periodo, fecha);
      }
    }
  ...
  /*
   * El estado mensual es un estado DE CARGOS, por lo que su fuente de verdad
   * debe ser cada cargo individual y no un acumulado por período.
   *
   * Esto evita el error de duplicar el pago cuando existen dos cargos con el
   * mismo período, por ejemplo:
   *   - SALDO_INICIAL 2026-01
   *   - MANTENIMIENTO 2026-01
   */
  const detalleMensual = useMemo(() => {
    return [...cargos]
      .sort((a, b) => {
        ...
        const saldoA =
          String(a.tipo_cargo || "").trim().toUpperCase() === "SALDO_INICIAL";
        const saldoB =
          String(b.tipo_cargo || "").trim().toUpperCase() === "SALDO_INICIAL";
      ...
      .map((cargo) => {
        ...
        const balance = Number(
          cargo.balance ?? totalCargo - montoAplicadoCargo
        );
        const periodoCargo =
          periodoValido(cargo.periodo) ||
          `${cargo.anio}-${String(cargo.mes || 0).padStart(2, "0")}`;
        ...
        const esSaldoInicial =
          String(cargo.tipo_cargo || "").trim().toUpperCase() ===
          "SALDO_INICIAL";

        const detalleArrastre = esSaldoInicial
          ? String(cargo.concepto || "")
              .match(/cuota pendiente\s+(.+)$/i)?.[1]
              ?.trim() || "Arrastre histórico"
          : "";

        const etiquetaPeriodo = esSaldoInicial
          ? `Saldo inicial (${detalleArrastre})`
          : `${MESES_NOMBRES[Number(cargo.mes)] || String(cargo.mes)} ${
              cargo.anio
            }`;
        ...
        return {
          ...
          totalCargo,
          montoAplicadoCargo,
          pagoAplicado: montoAplicadoCargo,
          fechaRealPago:
            fechasPagoPorCargo[Number(cargo.id)] ||
            (!esSaldoInicial ? fechasPagoPorPeriodo.get(periodoCargo) || null : null),
          balance,
          estadoMes,
        };
      });
  }, [cargos, fechasPagoPorCargo, fechasPagoPorPeriodo]);
  ...
  const totalPagosAplicadosAnio = detalleMensual.reduce(
    (sum, item) => sum + item.pagoAplicado,
    0
  );
    ...
    <main className="min-h-screen bg-slate-200 p-4 print:bg-white print:p-0">
      ...
      <section className="max-w-5xl mx-auto bg-white rounded-2xl shadow border p-6 print-card text-slate-900">
        ...
        <div className="mt-4 page-break-inside-avoid">
          ...
          <div className="mb-2 text-[10px] text-slate-500 page-break-inside-avoid">
            <strong>Fecha real pago:</strong> fecha bancaria en que el cargo quedó totalmente saldado.
            Si recibió varios abonos, se muestra la fecha en que el acumulado completó el cargo.
          </div>
            ...
            <table className="min-w-full text-sm print-table">
              <thead className="bg-slate-100">
                <tr>
                  ...
                  <th className="p-2 border text-center">Fecha real pago</th>
              ...
              <tbody>
                {aplicacionesPago.map((item) => {
                  ...
                  const fechaDetalle = cargo?.id
                    ? fechasPagoPorCargo[Number(cargo.id)] || pago?.fecha_pago || null
                  ...
                  return (
                    <tr key={item.id}>
                      ...
                      <td className="p-2 border text-center font-bold text-slate-700 whitespace-nowrap">
                        {fechaDetalle ? formatoFecha(fechaDetalle) : "-"}
                      </td>
        ...
        <div className="mt-5">
          <h3 className="font-black uppercase border-b pb-1 mb-2">
            Estado de cuenta de cargos del año {anioActual}
          </h3>

          <div className="mb-3 grid gap-2 sm:grid-cols-3 page-break-inside-avoid">
            <div className="rounded-lg border border-slate-200 bg-slate-50 p-3">
              <p className="text-[10px] font-black uppercase text-slate-500">
                Total recibido {anioActual}
              </p>
              <p className="mt-1 text-base font-black text-slate-900">
                RD$ {dinero(totalRecibidoAnio)}
              </p>
            </div>

            <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-3">
              <p className="text-[10px] font-black uppercase text-emerald-700">
                Aplicado a cargos generados
              </p>
              <p className="mt-1 text-base font-black text-emerald-900">
                RD$ {dinero(totalPagosAplicadosAnio)}
              </p>
            </div>
            ...
            <div className="rounded-lg border border-blue-200 bg-blue-50 p-3">
              ...
              <p className="mt-1 text-base font-black text-blue-900">
                RD$ {dinero(saldoFavorDisponible)}
              </p>
            </div>
          </div>
          ...
          {detalleMensual.some((item) => item.esSaldoInicial) && (
            <div className="mb-3 rounded-lg border border-blue-200 bg-blue-50 px-3 py-2 text-xs text-blue-900 page-break-inside-avoid">
              <strong>Saldo inicial:</strong> corresponde a una obligación anterior al
              inicio operativo de VAM (01/01/2026). Se presenta separada del cargo
              ordinario de enero y conserva la fecha real del pago aplicado.
            </div>
          )}
          ...
          <div className="overflow-auto border rounded-lg">
            <table className="min-w-full text-sm print-table">
              <thead className="bg-slate-100">
                <tr>
                  <th className="p-2 border">Período / concepto</th>
                  <th className="p-2 border">Cargo total</th>
                  <th className="p-2 border">Pago aplicado</th>
                  <th className="p-2 border">Fecha real pago</th>
              ...
              <tbody>
                {detalleMensual.map((item) => (
                  <tr key={item.id}>
                    <td className="p-2 border font-bold">
                      <div>{item.etiquetaPeriodo}</div>
                      {item.esSaldoInicial && (
                        <div className="mt-0.5 text-[9px] font-normal text-blue-700">
                          Apertura histórica
                        </div>
                      )}
                    </td>
                    ...
                    <td className="p-2 border text-right text-green-700 font-bold">
                      RD$ {dinero(item.pagoAplicado)}
                    </td>
                    <td className="p-2 border text-center font-bold text-slate-700 whitespace-nowrap">
                      {item.fechaRealPago ? formatoFecha(item.fechaRealPago) : "-"}
                    </td>
                    <td
