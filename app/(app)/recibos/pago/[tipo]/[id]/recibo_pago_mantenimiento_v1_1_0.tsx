...
export default function ReciboPagoPage() {
  ...
  async function cargarPagosAnualesReales(
    ...
    if (pagosError) {
      console.error("Error cargando pagos reales del año:", pagosError.message);
      setPagosAnualesReales([]);
      return;
    }

    const pagosBase = (pagosData || []) as any[];
    ...
    const aplicacionesPorPago = new Map<number, Array<{ periodo: string; monto: number }>>();
    ...
    for (const item of pagosBase) {
      ...
      // Un período explícito o identificado es la fuente principal para el reporte.
      // Esto evita clasificar por fecha bancaria pagos anticipados, como un pago de
      // abril depositado el 31 de marzo.
      if (periodosDirectos.length === 1) {
        filas.push({
          pago_id: pagoId,
          periodo: periodosDirectos[0],
      ...
      // Si el pago cubrió varios períodos, distribuye por las aplicaciones reales
      // y coloca cualquier excedente (saldo a favor generado) en el último período
      // cubierto, sin duplicar el monto recibido.
      ...
      const periodoFecha = periodoValido(String(item.fecha_pago || "").slice(0, 7));
      const periodoRespaldo = periodoDescripcion || periodoFecha;

      if (periodoRespaldo) {
        filas.push({
          pago_id: pagoId,
          periodo: periodoRespaldo,
          monto_recibido: montoPago,
          fecha_pago: item.fecha_pago || null,
          referencia: item.referencia || null,
          origen: item.origen || null,
