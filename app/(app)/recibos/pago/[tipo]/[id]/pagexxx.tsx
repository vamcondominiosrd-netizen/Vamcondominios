...
export default function ReciboPagoPage() {
  ...
  function extraerMesPagadoDesdeDescripcion(descripcion?: string | null) {
    const texto = String(descripcion || "").trim();
    const match = texto.match(/Pago\s+mantenimiento\s+(.+?)(?:\s+-\s+Unidad|$)/i);

    if (!match?.[1]) return "";

    const valor = match[1].trim();

    if (!valor || valor.toLowerCase() === "- unidad") return "";
  ...
  async function cargarReciboDesdeTablaPagos() {
    ...
    const pagoNormalizado: PagoRecibo = {
      ...
      mes_pagado: extraerMesPagadoDesdeDescripcion(pagoTabla.descripcion) || "-",
