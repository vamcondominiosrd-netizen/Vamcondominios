...
const MODULO_VERSION = "v1.1.4";
const MODULO_FECHA_VERSION = "11/09/2026";
...
export default function ReciboPagoPage() {
  ...
  async function cargarFechasRealesPorCargo(cargosBase: CargoPeriodico[]) {
    ...
    const { data: aplicacionesData, error: aplicacionesError } = await supabase
      .from("pagos_aplicaciones")
      .select("cargo_periodico_id, pago_id")
    ...
    for (const item of (pagosData || []) as Array<{
      ...
      if (item.fecha_pago) {
        fechaPorPago.set(Number(item.id), String(item.fecha_pago));
      }
    }
    ...
    for (const aplicacion of aplicaciones) {
      ...
      const fechaPagoReal = fechaPorPago.get(pagoIdAplicacion) || "";

      if (!cargoId || !fechaPagoReal) continue;

      // Si un cargo recibió varios abonos, conserva la fecha real más reciente.
      if (!resultado[cargoId] || fechaPagoReal > resultado[cargoId]) {
        resultado[cargoId] = fechaPagoReal;
      }
    }
   ...
   *
   * Fuente principal de la fecha: pagos_aplicaciones -> pagos.fecha_pago
   * (fechasPagoPorCargo).
   *
  ...
  const fechasPagoPorPeriodo = useMemo(() => {
    ...
    for (const item of pagosAnualesReales) {
      ...
      // Si hubo varios abonos para el mismo período, conserva el más reciente.
