...
export default function AprobacionTesoreroPage() {
  ...
  async function cargarPendientes(nombre: string) {
    setLoading(true);
  ...
  async function aprobar(g: Gasto) {
    const confirmar = confirm(`¿Aprobar este pago para enviarlo al presidente?\n\n${g.concepto}`);
    ...
    alert("Pago aprobado por tesorero.");
    cargarPendientes(condominioNombre);
  }

  async function rechazar(g: Gasto) {
    ...
    const { error } = await supabase
      .from("gastos")
      .update({
        estado: "Rechazado por tesorero",
        detalle_gasto: `${g.detalle_gasto || ""}\n\nMotivo rechazo tesorero: ${motivo}`,
      })
      .eq("id", g.id)
      .eq("condominio", condominioNombre);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="bg-white rounded-2xl border shadow-sm p-5">
          ...
          <h2 className="text-2xl font-bold text-red-700">
            RD${totalPendiente.toLocaleString("es-DO", { minimumFractionDigits: 2 })}
          </h2>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border shadow-sm overflow-hidden">
        <div className="p-4 border-b">
          <h2 className="font-bold">Pagos pendientes de aprobación</h2>
        </div>
                ...
                {gastos.length === 0 && (
                  <tr>
                    <td colSpan={6} className="px-4 py-8 text-center text-slate-500">
                      No hay pagos pendientes para aprobación del tesorero.
                    </td>
                  </tr>
                )}
              </tbody>
