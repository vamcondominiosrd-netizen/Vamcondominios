...
export default function ReporteGastosMensualesChequesPage() {
  ...
  async function cargarReporte(
    ...
    try {
      const { desde, hasta } =
        rangoPeriodo(periodoSeleccionado);

      const { data, error: gastosError } =
        await supabase
          .from("gastos")
          .select(`
            id,
      ...
      const detalle = ((data || []) as GastoDB[])
        .filter((row) =>
          tieneNumeroCheque(row.numero_cheque)
        )
        .map(mapearGasto)
  ...
  function obtenerNombreProveedor(row: GastoDB): string {
    ...
    return limpiarTexto(
      nombreRelacionado || row.proveedor,
      "Proveedor / beneficiario"
    );
  }

  function mapearGasto(row: GastoDB): DetalleGasto {
    const numeroCheque = limpiarTexto(
      row.numero_cheque,
      "-"
    );

    const fechaISO = String(
      row.fecha_pago || row.created_at || ""
    ).slice(0, 10);
    ...
    return {
      ...
      numeroCheque,
      numeroOrden:
        extraerNumeroCheque(numeroCheque),
      ...
      proveedor: obtenerNombreProveedor(row),
