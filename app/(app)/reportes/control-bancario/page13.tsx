...
async function obtenerCuentaBancariaActiva(
  ...
  async function cargarPropietariosPorUnidad(
    pCondominioId: number
  ): Promise<Map<number, string>> {
    const map = new Map<number, string>();

    async function intentarTabla(tabla: string) {
      try {
        const { data, error } = await supabase
          .from(tabla)
          .select("*")
        ...
        if (error) {
          console.warn(`No se pudo leer ${tabla}:`, error.message);
          return;
        }

        (data || []).forEach((row: any) => {
          const unidadId = obtenerUnidadId(row);
          const nombre = obtenerNombrePropietario(row);

          if (unidadId && nombre && !map.has(unidadId)) {
            map.set(unidadId, nombre);
          }
        });
      } catch (err) {
        console.warn(`Error leyendo ${tabla}:`, err);
      }
    }

    await intentarTabla("propietarios");
    await intentarTabla("propietarios_apartamentos");
  ...
  async function enriquecerIngresosConPropietarios(
    ...
    const propietariosMap = await cargarPropietariosPorUnidad(pCondominioId);
    ...
    movimientosIngreso.forEach((row: any) => {
      ...
      const propietario =
        (unidadId ? propietariosMap.get(unidadId) : "") ||
