...
export default function EstadoFinancieroBancarioPage() {
  ...
  async function cargarPeriodos(
    ...
    const registrosFinales: PeriodoBancario[] = [];
    const nombreActual = normalizarTexto(pCondominioNombre);

    try {
      const { data, error } = await supabase
        .from("periodos_bancarios")
        .select("*")
        .eq("condominio_id", pCondominioId)
        .order("periodo", { ascending: false });

      if (!error && data) {
        registrosFinales.push(...((data || []) as PeriodoBancario[]));
      } else if (error) {
        console.warn(
          "Error cargando periodos por condominio_id:",
          error.message
        );
      }
    } catch (err) {
      console.warn("Error cargando periodos exactos:", err);
    }

    try {
      const { data: dataTodos, error: errorTodos } = await supabase
        .from("periodos_bancarios")
        .select("*")
        .order("periodo", { ascending: false })
        .limit(500);

      if (!errorTodos && dataTodos) {
        const todos = (dataTodos || []) as PeriodoBancario[];

        const grupos = new Map<string, PeriodoBancario[]>();

        todos.forEach((row) => {
          if (!row.periodo) return;
          const lista = grupos.get(row.periodo) || [];
          lista.push(row);
          grupos.set(row.periodo, lista);
        });

        todos.forEach((row) => {
          const rowNombre = normalizarTexto(row.condominio);
          const rowCondominioId = Number(row.condominio_id ?? 0);
          const grupoPeriodo = grupos.get(row.periodo) || [];

          const coincideId = rowCondominioId === pCondominioId;
          const coincideNombre =
            nombreActual &&
            rowNombre &&
            (rowNombre.includes(nombreActual) ||
              nombreActual.includes(rowNombre));

          const periodoUnico = grupoPeriodo.length === 1;

          if (coincideId || coincideNombre || periodoUnico) {
            registrosFinales.push(row);
          }
        });
      } else if (errorTodos) {
        console.warn("Error cargando periodos alternativos:", errorTodos.message);
      }
    } catch (err) {
      console.warn("No se pudo hacer búsqueda adicional de periodos:", err);
    }

    const sinDuplicados = new Map<string, PeriodoBancario>();

    registrosFinales.forEach((p) => {
      if (!p.periodo) return;

      const existente = sinDuplicados.get(p.periodo);

      if (!existente) {
        sinDuplicados.set(p.periodo, p);
        return;
      }

      const existenteExacto =
        Number(existente.condominio_id ?? 0) === pCondominioId;
      const nuevoExacto = Number(p.condominio_id ?? 0) === pCondominioId;

      if (!existenteExacto && nuevoExacto) {
        sinDuplicados.set(p.periodo, p);
      }
    });
  ...
  async function buscarPeriodoBancario(
    ...
    try {
      ...
      if (!error && data) {
        return data as PeriodoBancario;
      }

      if (error) {
        console.warn("Error buscando periodo exacto:", error.message);
      }
    } catch (err) {
      console.warn("Error buscando periodo exacto:", err);
    }
    ...
    try {
      ...
      if (error) {
        console.warn("Error buscando periodo alternativo:", error.message);
        return null;
      }

      const registros = (data || []) as PeriodoBancario[];

      if (registros.length === 0) return null;

      const exacto = registros.find(
        (row) => Number(row.condominio_id ?? 0) === pCondominioId
      );

      if (exacto) return exacto;

      if (nombreActual && nombreActual !== "condominio") {
        const porNombre = registros.find((row) => {
          const rowNombre = normalizarTexto(row.condominio);
          if (!rowNombre) return false;

          return (
            rowNombre.includes(nombreActual) || nombreActual.includes(rowNombre)
          );
        });
      ...
      const sinCondominio = registros.filter(
        (row) =>
          row.condominio_id === null ||
          row.condominio_id === undefined ||
          Number(row.condominio_id) === 0
      );

      if (sinCondominio.length === 1) {
        return sinCondominio[0];
      }

      return null;
    } catch (err) {
      console.warn("Error en búsqueda alternativa de periodo:", err);
      return null;
    }
  }
  ...
  async function buscarBalanceInicialFallback(
    ...
    try {
      const { data, error } = await supabase
        .from("periodos_bancarios")
        .select("periodo, balance_final, estado, condominio_id, condominio")
        .eq("condominio_id", pCondominioId)
        .lt("periodo", periodo)
        .order("periodo", { ascending: false })
        .limit(1)
      ...
      if (!error && data) {
        const balance = toNumber(data.balance_final);
        if (balance > 0) return balance;
      }

      if (error) {
        console.warn("No se pudo buscar balance anterior exacto:", error.message);
      }
    } catch (err) {
      console.warn("Error buscando balance anterior exacto:", err);
    }
    ...
    try {
      ...
      if (error) {
        console.warn(
          "No se pudo buscar balance anterior alternativo:",
          error.message
        );
        return 0;
      }
      ...
      if (exacto) return toNumber(exacto.balance_final);
      ...
      if (nombreActual && nombreActual !== "condominio") {
        ...
        if (porNombre) return toNumber(porNombre.balance_final);
      }

      const periodoMasReciente = registros[0]?.periodo;
      const registrosDelPeriodoMasReciente = registros.filter(
        (row) => row.periodo === periodoMasReciente
      );

      if (registrosDelPeriodoMasReciente.length === 1) {
        return toNumber(registrosDelPeriodoMasReciente[0].balance_final);
      }
      ...
      if (sinCondominio.length === 1) {
        return toNumber(sinCondominio[0].balance_final);
      }

      return 0;
    } catch (err) {
      console.warn("Error buscando balance anterior alternativo:", err);
      return 0;
    }
  }
  ...
  async function consultarReporte(
    ...
    try {
      const ingresosData = await cargarIngresos(periodo, pCondominioId);
      const gastosData = await cargarGastos(periodo, pCondominioId);
  ...
  async function cargarIngresos(
    ...
    const tablas = [
      {
        nombre: "pagos_mantenimiento",
        etiqueta: "Pagos mantenimiento",
      },
      {
        nombre: "pagos_identificados",
        etiqueta: "Pagos identificados",
      },
      {
        nombre: "pagos",
        etiqueta: "Pagos",
    ...
    const resultados: MovimientoIngreso[] = [];
    ...
    for (const tabla of tablas) {
      ...
      if (error) {
        console.warn(`No se pudo leer ${tabla.nombre}:`, error.message);
        continue;
      }

      const filtrados = (data || [])
        .filter((row: any) => perteneceAlPeriodo(row, periodo))
        .filter((row: any) => {
        ...
        .map((row: any, index: number) => {
          ...
          return {
            ...
            concepto: limpiarTexto(
              row.concepto || row.descripcion || row.observacion,
    ...
    const sinDuplicados = new Map<string, MovimientoIngreso>();

    resultados.forEach((item) => {
      const key = `${item.fecha}-${item.referencia}-${item.monto}-${item.unidad}-${item.fuente}`;
      if (!sinDuplicados.has(key)) sinDuplicados.set(key, item);
    });

    return Array.from(sinDuplicados.values()).sort((a, b) =>
  ...
  async function cargarGastos(
    ...
    const tablas = [
      ...
      {
        nombre: "solicitudes_pago",
        etiqueta: "Solicitudes de pago",
      },
    ];

    const resultados: MovimientoGasto[] = [];
      ...
      const filtrados = (data || [])
        ...
        .map((row: any, index: number) => {
          ...
          return {
            ...
            concepto: limpiarTexto(
              row.concepto || row.descripcion || row.observacion || row.detalle,
              "Gasto operativo"
            ),
            metodo_pago: limpiarTexto(
              row.metodo_pago ||
                row.forma_pago ||
                row.tipo_pago ||
                row.metodo ||
    ...
    const sinDuplicados = new Map<string, MovimientoGasto>();

    resultados.forEach((item) => {
      const key = `${item.fecha}-${item.referencia}-${item.monto}-${item.proveedor}-${item.fuente}`;
      if (!sinDuplicados.has(key)) sinDuplicados.set(key, item);
    });
      ...
      <div className="mx-auto max-w-7xl space-y-5">
        ...
        {esPeriodoSinCierre && (
          <div className="no-print rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">
            Este periodo no existe todavía en{" "}
            <strong>periodos_bancarios</strong>. El reporte se está mostrando
            calculado con los movimientos encontrados y el último balance
            anterior disponible.
          </div>
        )}
