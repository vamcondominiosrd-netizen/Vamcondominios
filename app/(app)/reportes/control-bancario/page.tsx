...
export default function EstadoFinancieroBancarioPage() {
  ...
  const totalCargosBancariosDetalle = useMemo(
    () =>
      gastos
        .filter((item) => item.esCargoBancario)
        .reduce((acc, item) => acc + toNumber(item.monto), 0),
    [gastos]
  );

  const totalGastosOperativosDetalle = useMemo(
    () =>
      gastos
        .filter((item) => !item.esCargoBancario)
        .reduce((acc, item) => acc + toNumber(item.monto), 0),
...
async function obtenerCuentaBancariaActiva(
  ...
  function fechaISO(value: any): string {
    if (!value) return "";
    return String(value).slice(0, 10);
  }

  function extraerIdNumerico(value: any): number | null {
    const n = Number(value);
    return Number.isFinite(n) && n > 0 ? n : null;
  }

  function valorTexto(row: any, campos: string[], fallback = ""): string {
  ...
  function obtenerUnidadId(row: any): number | null {
    const campos = [
      "unidad_id",
      "id_unidad",
      "apartamento_id",
    ...
    for (const campo of campos) {
      const id = extraerIdNumerico(row?.[campo]);
      if (id) return id;
    }
  ...
  function obtenerNombrePropietario(row: any): string {
    return valorTexto(row, [
      "propietario",
      "nombre_propietario",
      "propietario_nombre",
      "nombre",
  ...
  function esCargoImpuestoBancario(row: any): boolean {
  ...
  async function cargarPagosPeriodo(
    periodo: string,
    pCondominioId: number,
    movimientosIngreso: any[]
  ): Promise<any[]> {
    const { desde, hasta } = rangoPeriodo(periodo);
    const pagos: any[] = [];
    const vistos = new Set<string>();

    function agregarFilas(rows: any[] | null | undefined) {
      (rows || []).forEach((row: any) => {
        const key = String(row.id ?? `${row.fecha_pago}-${row.monto}-${row.referencia}`);
    ...
    const pagoIds = Array.from(
      new Set(
        movimientosIngreso
          .map((row: any) => extraerIdNumerico(row?.referencia_id))
    ...
    try {
      if (pagoIds.length > 0) {
        ...
        if (!error) agregarFilas(data);
      }
    } catch (err) {
      console.warn("No se pudieron cargar pagos por referencia_id:", err);
    }
    ...
    try {
      ...
      console.warn("No se pudieron cargar pagos por periodo:", err);
    }
    ...
    try {
      ...
      console.warn("No se pudieron cargar pagos por fecha:", err);
    }

    return pagos;
  }

  async function cargarUnidadesPorIds(unidadIds: number[]): Promise<Map<number, any>> {
    const map = new Map<number, any>();

    if (unidadIds.length === 0) return map;
    ...
    try {
      ...
      if (error) {
        console.warn("No se pudieron cargar unidades:", error.message);
        return map;
      }

      (data || []).forEach((row: any) => {
        const id = extraerIdNumerico(row?.id);
        if (id) map.set(id, row);
      });
    } catch (err) {
      console.warn("Error cargando unidades:", err);
    }

    return map;
  }

  function normalizarCodigoUnidad(value: any): string {
    return String(value || "")
      .toUpperCase()
      .replace(/\s+/g, "")
      .trim();
  ...
  async function cargarPropietariosApartamentosPorCodigo(
    pCondominioId: number
    ...
    try {
      const { data, error } = await supabase
        ...
        .select("condominio_id, no_apartamento, nombre_propietario")
        .eq("condominio_id", pCondominioId)
        .limit(10000);

      if (error) {
        console.warn("No se pudieron cargar propietarios_apartamentos:", error.message);
        return map;
      }

      (data || []).forEach((row: any) => {
        const codigo = normalizarCodigoUnidad(row?.no_apartamento);
        const nombre = limpiarTexto(row?.nombre_propietario, "");
      ...
      console.warn("Error cargando propietarios_apartamentos:", err);
    }

    return map;
  }

  function buscarPagoParaMovimiento(row: any, pagos: any[]): any | null {
    const referenciaId = extraerIdNumerico(row?.referencia_id);

    if (referenciaId) {
      const porId = pagos.find((p) => Number(p?.id) === referenciaId);
      if (porId) return porId;
    }

    const fechaMov = fechaISO(row?.fecha_movimiento || row?.fecha_banco || row?.created_at);
    const montoMov = toNumber(row?.monto);
    const referenciaMov = normalizarTexto(
      row?.referencia_banco || row?.numero_documento || ""
    );

    return (
      pagos.find((p) => {
        const mismaFecha = fechaISO(p?.fecha_pago || p?.created_at) === fechaMov;
        const mismoMonto = Math.abs(toNumber(p?.monto) - montoMov) < 0.01;
        const referenciaPago = normalizarTexto(p?.referencia || p?.numero_documento || "");
        const mismaReferencia =
          referenciaMov && referenciaPago ? referenciaMov === referenciaPago : true;

        return mismaFecha && mismoMonto && mismaReferencia;
      }) || null
    );
  }

  async function enriquecerIngresosConPropietarios(
    periodo: string,
    pCondominioId: number,
    movimientosIngreso: any[]
  ): Promise<Map<string, { propietario: string; unidad: string }>> {
    const map = new Map<string, { propietario: string; unidad: string }>();

    const pagos = await cargarPagosPeriodo(periodo, pCondominioId, movimientosIngreso);
    const unidadIds = Array.from(
      new Set(
        pagos
          .map((p: any) => obtenerUnidadId(p))
          .filter(Boolean) as number[]
      )
    );

    const unidadesMap = await cargarUnidadesPorIds(unidadIds);
    const propietariosPorCodigo = await cargarPropietariosApartamentosPorCodigo(pCondominioId);

    movimientosIngreso.forEach((row: any) => {
      const key = String(row.id ?? `${row.fecha_movimiento}-${row.monto}-${row.descripcion}`);
      const pago = buscarPagoParaMovimiento(row, pagos);
      const unidadId = obtenerUnidadId(pago);
      const unidad = unidadId ? unidadesMap.get(unidadId) : null;

      const unidadNombre =
        valorTexto(unidad, ["codigo", "nombre", "unidad", "apartamento"], "") ||
        valorTexto(pago, ["unidad", "apartamento", "codigo_unidad"], "");

      const codigoUnidad = normalizarCodigoUnidad(unidadNombre);

      const propietario =
        (codigoUnidad ? propietariosPorCodigo.get(codigoUnidad) : "") ||
        valorTexto(unidad, ["propietario_nombre"], "") ||
        obtenerNombrePropietario(pago) ||
        limpiarTexto(row?.beneficiario, "N/D");

      map.set(key, {
        propietario: propietario || "N/D",
        unidad: unidadNombre || "N/D",
      });
    });
  ...
  async function cargarIngresos(
    ...
    try {
      ...
      const ingresosBanco = movimientos.filter((row: any) =>
        tipoMovimientoEs(row, "INGRESO")
      );

      const propietariosMap = await enriquecerIngresosConPropietarios(
        periodo,
        pCondominioId,
        ingresosBanco
      );

      return ingresosBanco
        .map((row: any) => {
          const key = String(row.id ?? `${row.fecha_movimiento}-${row.monto}-${row.descripcion}`);
          const extra = propietariosMap.get(key);

          return {
            id: `banco-${row.id}`,
            fecha: formatDate(row.fecha_movimiento || row.fecha_banco || row.created_at),
            unidad: extra?.unidad || "N/D",
            propietario: extra?.propietario || limpiarTexto(row.beneficiario, "N/D"),
    ...
    try {
      ...
      return movimientos
        ...
        .map((row: any) => ({
          ...
          esCargoBancario: esCargoImpuestoBancario(row),
        }))
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <tbody>
                      ...
                      <tr className="border-b border-slate-100">
                        <td className="px-5 py-3 font-medium text-slate-700">
                          Menos gastos operativos pagados
                        </td>
                        <td className="px-5 py-3 text-right font-semibold text-red-700">
                          {formatMoney(totalGastosOperativosDetalle)}
                        </td>
                      </tr>

                      <tr className="border-b border-slate-100">
                        <td className="px-5 py-3 font-medium text-slate-700">
                          Menos impuestos / cargos bancarios
                        </td>
                        <td className="px-5 py-3 text-right font-semibold text-red-700">
                          {formatMoney(totalCargosBancariosDetalle)}
                        </td>
                      </tr>
                        ...
                        gastos.map((item) => (
                          ...
                          >
                            ...
                            <td className="px-4 py-3">
                              {item.esCargoBancario ? "Impuesto / cargo bancario" : item.categoria}
                            </td>
                            <td className="px-4 py-3">{item.concepto}</td>
