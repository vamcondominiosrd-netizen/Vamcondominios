...
export default function ConfiguracionCargosPage() {
  ...
  async function generarCargosPeriodo() {
    ...
    if (resumenAntes && resumenAntes.cantidad > 0) {
      alert(
        `El período ${periodoSeleccionado} ya fue generado.\n\n` +
          `Cargos registrados: ${resumenAntes.cantidad}\n` +
          `Total facturado: RD$ ${dinero(resumenAntes.total)}\n\n` +
          "No se permite ejecutar nuevamente la generación para este período."
      );
      return;
    }
  ...
  const bloqueoGeneracion = generando || loading || periodoGenerado;
        ...
        <div className="grid grid-cols-1 gap-4 xl:grid-cols-4">
          ...
          <div className="xl:col-span-1 rounded-2xl border bg-slate-50 p-5">
            ...
            <p className="mt-2 text-sm leading-relaxed text-slate-600">
              {periodoGenerado ? (
                <>
                  El período <strong>{periodoSeleccionado}</strong> ya fue generado
                  con <strong>{resumenCantidad} cargos</strong> por un total de{" "}
                  <strong>RD$ {dinero(resumenTotal)}</strong>.
                </>
              ) : (
                <>
                  El botón generará cargos para{" "}
                  <strong>{nombrePeriodo(periodoSeleccionado)}</strong> usando el
                  día de generación configurado:{" "}
                  <strong>{diaGeneracion || 1}</strong>.
                </>
              )}
            </p>
