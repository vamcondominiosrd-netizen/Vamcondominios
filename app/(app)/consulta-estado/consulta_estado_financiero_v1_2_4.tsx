...
export default function ConsultaEstadoPage() {
  ...
  const [condominioRnc, setCondominioRnc] = useState("");
  ...
  useEffect(() => {
    ...
    setCondominioRnc(idGuardado === "1" ? "430289002" : "");
    ...
    // El RNC es propio de cada condominio, no de VAM.
    // Se recupera de su registro cuando está disponible; no se inventa para otros condominios.
    async function cargarIdentificacionCondominio() {
      const { data, error } = await supabase
        .from("condominios")
        .select("*")
        .eq("id", Number(idGuardado))
        .maybeSingle();

      if (error || !data) {
        if (error) console.warn("No se pudo consultar el RNC del condominio:", error.message);
        return;
      }

      const registro = data as Record<string, unknown>;
      const nombre = String(registro.nombre || "").trim();
      const rnc = String(registro.rnc || registro.RNC || "").trim();
      if (nombre) setCondominioNombre(nombre);
      if (rnc) setCondominioRnc(rnc);
    }
    void cargarIdentificacionCondominio();
  }, []);
    ...
    <PageContainer>
      ...
      {impresionMontada && cargos.length > 0 && unidadSeleccionada && createPortal(
        <article className="vam-print-sheet" aria-label="Estado de cuenta financiero imprimible del propietario">
          <div className="vam-print-heading">
            <p className="vam-print-condo">{condominioNombre || (condominioId ? `Condominio ID ${condominioId}` : "Condominio no seleccionado")}</p>
            {condominioRnc && <p className="vam-print-rnc">RNC: {condominioRnc}</p>}
            <h1>ESTADO DE CUENTA</h1>
            <p className="vam-print-subtitle">Estado financiero del propietario</p>
            <div className="vam-print-heading-bottom">
              <span>Períodos consultados: {periodosConsultados}</span>
      ...
      <style jsx global>{`
        ...
        @media print {
          ...
          .vam-print-heading { text-align: center; border-bottom: 2px solid #10294d; padding-bottom: 9px; margin-bottom: 10px; break-inside: avoid; }
          .vam-print-heading p { margin: 3px 0 0; }
          .vam-print-condo { font-size: 15px; line-height: 1.2; font-weight: 900; color: #10294d; overflow-wrap: anywhere; text-transform: uppercase; }
          .vam-print-rnc { font-size: 8px; line-height: 1.2; color: #475569; letter-spacing: 0.25px; }
          .vam-print-heading h1 { margin: 7px 0 0; font-size: 17px; letter-spacing: 0.5px; font-weight: 900; color: #10294d; }
          .vam-print-subtitle { font-size: 9px; font-weight: 700; color: #334155; }
          .vam-print-heading-bottom { display: flex; justify-content: space-between; gap: 12px; margin-top: 9px; font-size: 8px; color: #475569; text-align: left; }
          .vam-print-heading-bottom span:last-child { text-align: right; }
