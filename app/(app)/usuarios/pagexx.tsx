...
export default function DirectivaUsuariosPage() {
  ...
  const [generandoCodigoId, setGenerandoCodigoId] = useState<number | null>(null);
  const [codigoGenerado, setCodigoGenerado] = useState<CodigoActivacionGenerado | null>(null);
  ...
  async function generarCodigoActivacion(item: Directiva) {
    setMensaje("");
    setCodigoCopiado(false);

    if (item.estado !== "Activo") {
      setMensaje("El miembro debe estar activo para generar un código de activación.");
      return;
    }

    const cedulaNormalizada = String(item.cedula || "").replace(/\D/g, "");
    if (cedulaNormalizada.length !== 11) {
      setMensaje(`Debe registrar una cédula válida de 11 dígitos para ${item.nombre}.`);
      return;
    }

    if (!String(item.correo || "").trim()) {
      setMensaje(`Debe registrar el correo electrónico de ${item.nombre} antes de generar el código.`);
      return;
    }

    setGenerandoCodigoId(item.id);

    try {
      const { data, error } = await supabase.rpc(
        "generar_codigo_activacion_directiva",
        {
          p_directiva_id: item.id,
          p_vigencia_dias: 15,
        }
      );

      if (error) {
        setMensaje("Error generando código de activación: " + error.message);
        return;
      }
      ...
      if (!respuesta?.ok) {
        setMensaje(
          respuesta?.mensaje ||
            "No fue posible generar el código de activación."
        );
        return;
      }

      setCodigoGenerado({
        codigo: String(respuesta.codigo || ""),
        vence_at: String(respuesta.vence_at || ""),
        directiva_id: Number(respuesta.directiva_id || item.id),
        condominio_id: Number(respuesta.condominio_id || item.condominio_id),
        nombre: String(respuesta.nombre || item.nombre),
        cargo: String(respuesta.cargo || item.cargo),
        correo: String(respuesta.correo || item.correo || ""),
      });

      setMensaje("Código de activación generado correctamente.");
    } catch (error: unknown) {
      setMensaje(
  ...
  async function copiarCodigoActivacion() {
    ...
    try {
      ...
      setCodigoCopiado(false);
      setMensaje("No fue posible copiar el código. Puede copiarlo manualmente.");
    }
  }
  ...
  return (
    <PageContainer>
      ...
      {codigoGenerado && (
        <SectionCard
          title="Código de activación de Directiva"
          subtitle="Entregue este código únicamente al miembro indicado. Si genera uno nuevo, el anterior quedará revocado."
        >
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-[1fr_auto] lg:items-center">
            <div>
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-black text-slate-900">{codigoGenerado.nombre}</p>
                  <p className="text-sm text-slate-500">
                    {codigoGenerado.cargo} · {codigoGenerado.correo}
                  </p>
                </div>
              </div>
              ...
              <div className="mt-4 rounded-2xl border border-blue-200 bg-blue-50 p-4">
                ...
                <p className="mt-2 text-xs text-slate-600">
                  Vence: {formatearFechaHora(codigoGenerado.vence_at)}
                </p>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-2">
              <button
                type="button"
                onClick={() => void copiarCodigoActivacion()}
                className="inline-flex items-center justify-center gap-2 rounded-xl bg-blue-700 px-4 py-3 text-sm font-bold text-white hover:bg-blue-800"
              >
                    ...
                    <tr key={d.id} className="bg-white hover:bg-slate-50">
                      ...
                      <td className="px-4 py-3 text-center">
                        <div className="flex flex-wrap justify-center gap-2">
                          <button
                            type="button"
                            onClick={() => void generarCodigoActivacion(d)}
                            disabled={
                              generandoCodigoId === d.id ||
                              d.estado !== "Activo" ||
                              String(d.cedula || "").replace(/\D/g, "").length !== 11 ||
                            ...
                            title={
                              d.estado !== "Activo"
                                ...
                                : String(d.cedula || "").replace(/\D/g, "").length !== 11
                                  ? "Debe registrar una cédula válida de 11 dígitos"
                                  : !String(d.correo || "").trim()
                                    ? "Debe registrar un correo electrónico"
                                    : "Generar código de activación"
                            }
                            className="rounded-lg bg-emerald-700 px-3 py-1 text-xs font-bold text-white hover:bg-emerald-800 disabled:cursor-not-allowed disabled:bg-slate-300"
                          >
                            {generandoCodigoId === d.id ? "Generando..." : "Generar código"}
                          </button>
