...
import { useCondominio } from "@/app/context/CondominioContext";
...
export default function PagosMantenimientoPage() {
  const { condominioSeleccionado } = useCondominio();
  ...
  useEffect(() => {
    if (condominioSeleccionado) {
      setCondominio(condominioSeleccionado);
    }
 ...
 async function cargarPagos() {
  ...
  async function guardarPago(e: React.FormEvent) {
    ...
    // Mantener el condominio seleccionado para continuar registrando pagos.
    setApartamento("");
    setFechaPago("");
    setMesPagado("");
    setMontoPagado("");
  ...
  const pagosFiltrados = pagos.filter((p) => {
    return (
      condominioSeleccionado === "" ||
      p.condominio === condominioSeleccionado
    );
  });
