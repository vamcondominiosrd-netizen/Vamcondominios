...
export default function PagosMantenimientoPage() {
  ...
  async function obtenerMesesPagados(idCondominio: string, idUnidad: number, montoPago: number) {
    const { data, error } = await supabase
    ...
    if (error) {
      throw new Error("Error buscando los cargos pendientes para identificar el mes pagado: " + error.message);
    }
    ...
    for (const cargo of data || []) {
      ...
      periodos.push(String((cargo as any).periodo || ""));
    ...
    return periodosUnicos.map(nombreMesPeriodo).join(", ");
  }
  ...
  async function guardarPago(e: React.FormEvent) {
    ...
    try {
      ...
      const mesesPagados = await obtenerMesesPagados(condominioId, unidad.id, montoNumerico);
      const descripcionPago = mesesPagados
        ? `Pago mantenimiento ${mesesPagados}`
        : `Pago mantenimiento - Unidad ${unidad.codigo}`;
