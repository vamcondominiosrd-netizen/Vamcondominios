...
const TIPO_RECIBO_PAGO = "mantenimiento";

function rutaReciboPago(id: number | string) {
  return `/recibos/pago/${TIPO_RECIBO_PAGO}/${id}`;
}
...
export default function PagosMantenimientoPage() {
  ...
  const [cuotaOrdinaria, setCuotaOrdinaria] = useState<number | null>(null);
  ...
  async function cargarConfiguracionCargos(id: string) {
    ...
    if (error) {
      ...
      setCuotaOrdinaria(null);
      return null;
    }

    const cuota = data?.cuota_ordinaria != null ? Number(data.cuota_ordinaria) : null;
    setCuotaOrdinaria(cuota);
    return cuota;
  }
  ...
  async function cargarEstadoUnidad(idUnidad: string) {
    if (!condominioId || !idUnidad) {
      setCargosAbiertos([]);
      setCreditos([]);
      return { cargos: [] as CargoAbierto[], creditos: [] as CreditoUnidad[] };
    }
    ...
    const cargosFinales = (cargos as CargoAbierto[]) || [];
    const creditosFinales = errorCreditos ? [] : (creditosData as CreditoUnidad[]) || [];

    setCargosAbiertos(cargosFinales);
    setCreditos(creditosFinales);

    return { cargos: cargosFinales, creditos: creditosFinales };
  }
  ...
  async function seleccionarUnidad(idUnidad: string) {
    ...
    const [{ cargos }, cuotaConfigurada] = await Promise.all([
      cargarEstadoUnidad(idUnidad),
      cuotaOrdinaria != null
        ? Promise.resolve(cuotaOrdinaria)
        : cargarConfiguracionCargos(condominioId),
    ]);
    ...
    const cuotaRespaldo = Number(unidad.cuota_mensual_actual || 0);
    const montoSugerido =
      balancePendiente > 0
        ? balancePendiente
        : Number(cuotaConfigurada || 0) > 0
        ? Number(cuotaConfigurada)
        : cuotaRespaldo;

    setMonto(montoSugerido > 0 ? String(montoSugerido) : "");
  }
  ...
  async function guardarPago(e?: React.FormEvent, imprimirDespues = false) {
    ...
    try {
      ...
      if (imprimirDespues) {
        window.open(rutaReciboPago(pagoInsertado.id), "_blank", "noopener,noreferrer");
      }
  ...
  return (
    <PageContainer>
      <ModuleMenu
        ...
        subtitle="Registro único de pagos, banco, recibos, créditos y cuadre diario."
        tone="green"
        items={[
          { href: "/pagos-mantenimiento", label: "Pago Mantenimiento", icon: Banknote },
      ...
      <ModuleToolbar
        title="Pagos de Mantenimiento"
        subtitle={`Registro único de pagos de mantenimiento. Condominio: ${
      ...
      {mensaje && (
        <div className="rounded-xl border border-blue-200 bg-blue-50 p-4 text-sm text-blue-800">
          ...
          {ultimoPagoId && (
            <Link
              href={rutaReciboPago(ultimoPagoId)}
      ...
      <div className="grid grid-cols-1 gap-5 xl:grid-cols-3">
        <section className="xl:col-span-2">
          ...
          >
            <form
              onSubmit={(e) => guardarPago(e, false)}
                    ...
                    {unidadesFiltradas.map((u) => (
                      ...
                      >
                        ...
                        <p className="text-xs text-slate-500">
                          ...
                          {dinero(cuotaOrdinaria ?? u.cuota_mensual_actual)}
                        </p>
                      </button>
                    ))}
                  </div>
            ...
            {!unidadSeleccionada ? (
              ...
              <div className="space-y-4">
                ...
                <InfoLine
                  ...
                  value={`RD$ ${dinero(cuotaOrdinaria ?? unidadSeleccionada.cuota_mensual_actual)}`}
                />
