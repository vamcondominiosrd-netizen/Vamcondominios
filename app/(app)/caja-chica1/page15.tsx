...
function textoSeguro(valor: string | number | null | undefined) {
  return String(valor ?? "")
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;")
...
function imprimirConstanciaCajaChica(registro: CajaChica) {
  const detalleHtml = registro.detalle_gasto
    ? `
      <div class="bloque bloque-detalle">
        <div class="etiqueta">Detalle</div>
        <div class="texto-largo">${textoSeguro(registro.detalle_gasto)}</div>
      </div>
    `
    : "";

  const comprobanteHtml = registro.comprobante
    ? `
      <div class="campo campo-completo">
        <div class="etiqueta">Comprobante / Factura</div>
        <div class="valor">${textoSeguro(registro.comprobante)}</div>
      </div>
    `
    : "";
  ...
  const html = `
    ...
    <html lang="es">
      <head>
        ...
        <title>Constancia Caja Chica CC-${textoSeguro(registro.id)}</title>
        <style>
          * {
        ...
        <main class="comprobante">
          <section class="encabezado">
            ...
            <div class="numero">
              ...
              <strong>CC-${textoSeguro(registro.id)}</strong>
            </div>
          </section>

          <section class="grid">
            <div class="campo">
              <div class="etiqueta">Condominio</div>
              <div class="valor">${textoSeguro(registro.condominio || "-")}</div>
            </div>
            <div class="campo">
              <div class="etiqueta">Fecha Registro</div>
              <div class="valor">${textoSeguro(fechaRD(registro.fecha))}</div>
            </div>
            <div class="campo">
              <div class="etiqueta">Estado</div>
              <div class="valor">${textoSeguro(registro.estado || "registrado")}</div>
            </div>
          </section>

          <section class="grid-dos">
            <div class="campo">
              <div class="etiqueta">Pagado a / Responsable</div>
              <div class="valor">${textoSeguro(registro.responsable || "-")}</div>
            </div>
            <div class="campo">
              <div class="etiqueta">Monto</div>
              <div class="valor monto">RD$ ${textoSeguro(dinero(registro.monto))}</div>
            </div>
          </section>

          <section class="bloque">
            <div class="etiqueta">Concepto</div>
            <div class="texto-largo">${textoSeguro(registro.concepto || "-")}</div>
          </section>
          ...
          <section class="firmas">
            <div class="firma">
              Aprobado por
              <span class="subfirma">Firma autorizada</span>
            </div>
            <div class="firma">
              Recibido por
              <span class="subfirma">Firma del responsable</span>
            </div>
          </section>
  ...
  if (!ventana) {
    alert(
      "No se pudo abrir la ventana de impresión. Verifica que el navegador no esté bloqueando ventanas emergentes."
    );
    return;
  }
              ...
              {ultimosGastos.map((g) => (
                <tr key={g.id} className="bg-white hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3 text-center">
                    <button
                      type="button"
                      onClick={() => imprimirConstanciaCajaChica(g)}
                      className="inline-flex items-center justify-center gap-1 rounded-lg bg-purple-700 px-3 py-1 text-xs font-bold text-white hover:bg-purple-800"
                    >
