...
function crearOpcionesMarcasDesdeLista(lista: string[]) {
  return lista
...
export default function VehiculosPage() {
  ...
  const [marcas, setMarcas] = useState<Opcion[]>(crearOpcionesMarcasDesdeLista(marcasComunes))
  ...
  async function cargarDatos(condominioParam?: Opcion | null) {
    ...
    if (actual) setCondominioActual(actual)
  ...
  async function cargarVehiculos(condominioParam?: Opcion | null) {
    ...
    if (actual?.id) query = query.eq('condominio_id', actual.id)
    ...
    if (error) {
      ...
      if (actual?.id) fallbackQuery = fallbackQuery.eq('condominio_id', actual.id)

      const fallback = await fallbackQuery

      if (!fallback.error) setVehiculos((fallback.data || []) as Vehiculo[])
      return
    }
  ...
  async function cargarMarcas() {
    ...
    if (error) {
      ...
      setMarcas(crearOpcionesMarcasDesdeLista(marcasComunes))
      return
    }

    const nombresDb = (data || []).map((item: any) => String(item.nombre || '').trim()).filter(Boolean)
    const nombresUnicos = Array.from(new Set([...nombresDb, ...marcasComunes]))

    setMarcas(crearOpcionesMarcasDesdeLista(nombresUnicos))
  }

  async function agregarMarca(nombre: string, index: number) {
    const nombreLimpio = nombre.trim()

    if (!nombreLimpio) return

    const existe = marcas.some(
      (marca) => marca.nombre.trim().toLowerCase() === nombreLimpio.toLowerCase()
    )

    if (existe) {
      actualizarVehiculoFormulario(index, 'marca', nombreLimpio)
      return
    }

    const { data, error } = await supabase
      .from('vehiculos_marcas')
      .insert({ nombre: nombreLimpio, estado: 'Activo' })
      .select()
      .single()

    if (error) {
      const confirmar = window.confirm(
        `No se pudo guardar la marca en el catálogo: ${error.message}\n\n¿Deseas usarla solo en este registro?`
      )

      if (confirmar) {
        setMarcas((prev) =>
          crearOpcionesMarcasDesdeLista([...prev.map((m) => m.nombre), nombreLimpio])
        )
        actualizarVehiculoFormulario(index, 'marca', nombreLimpio)
      }
    ...
    await cargarMarcas()
    actualizarVehiculoFormulario(index, 'marca', data?.nombre || nombreLimpio)
  }
  ...
  const resumen = useMemo(() => {
    ...
    const tagsActivos = vehiculos.filter(
      (v) => v.numero_tarjeta_acceso && v.estado_tarjeta_acceso === 'Activa'
    ).length
    const tagsBloqueados = vehiculos.filter(
      (v) => v.estado_tarjeta_acceso === 'Bloqueada'
    ).length

    return { total, activos, bloqueados, tagsActivos, tagsBloqueados }
  }, [vehiculos])
  ...
  function seleccionarUnidad(id: string) {
    ...
    const propietarioNombre =
      item?.raw?.propietario || item?.raw?.nombre_propietario || item?.raw?.titular || ''
  ...
  async function guardarVehiculo() {
    ...
    const crearPayload = (v: VehiculoFormulario) => ({
      ...
      tiene_parqueo: !!v.parqueo_asignado,
      parqueo_asignado: v.parqueo_asignado || null,
      tipo_parqueo: v.parqueo_asignado ? 'Referencia' : null,
      estado_parqueo: v.parqueo_asignado ? 'Informativo' : null,
    ...
    <div className="min-h-screen bg-slate-50 p-4 md:p-6">
      <div className="mx-auto max-w-7xl space-y-6">
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                Registro de vehículos del condominio actual, asignación de TAG/tarjeta y
                preparación para integración con control de acceso externo.
              </p>
        ...
        <div className="grid gap-4 md:grid-cols-5">
          ...
          <ResumenCard titulo="TAG activos" valor={resumen.tagsActivos} detalle="Tarjetas activas" />
          <ResumenCard
            titulo="TAG bloqueados"
            valor={resumen.tagsBloqueados}
            detalle="Tarjetas/TAG bloqueados"
            alerta={resumen.tagsBloqueados > 0}
          />
        </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <div className="grid gap-3 md:grid-cols-5">
            <input
              ...
              placeholder="Buscar placa, unidad, propietario, marca, TAG..."
            ...
            <select
              ...
              className="rounded-xl border border-slate-300 px-3 py-2 text-sm outline-none focus:border-blue-500"
            >
              <option value="">Estado TAG</option>
              ...
              <table className="min-w-full divide-y divide-slate-200 text-sm">
                <thead className="bg-slate-100">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-left font-semibold text-slate-700">Vehículo</th>
                ...
                <tbody className="divide-y divide-slate-100">
                  {vehiculosFiltrados.map((v) => (
                    <tr key={v.id} className="hover:bg-slate-50">
                      ...
                      <td className="px-4 py-3">
                        <div className="font-medium text-slate-900">
                          {textoSeguro(v.parqueo_asignado)}
                        </div>
                        <div className="text-xs text-slate-500">
                          Código: {textoSeguro(v.codigo_dispositivo_acceso)}
                        </div>
                      </td>
              ...
              <div className="flex items-center justify-between gap-4">
                <div>
                  ...
                  <p className="text-sm text-slate-500">
                    ...
                    varios vehículos con su TAG/tarjeta.
                  </p>
                </div>
                  ...
                  {vehiculosFormulario.map((vehiculoItem, index) => (
                    ...
                    >
                      <div className="mb-4 flex items-center justify-between gap-3">
                        <div>
                          ...
                          <p className="text-xs text-slate-500">
                            Registra placa, marca, TAG/tarjeta y datos del conductor.
                          </p>
                        </div>
                      ...
                      <div className="grid gap-4 md:grid-cols-4">
                        ...
                        <Campo label="Marca">
                          <select
                            ...
                            onChange={(e) => {
                              ...
                              if (value === '__nueva__') {
                                const nuevaMarca = window.prompt('Escribe la nueva marca:')

                                if (nuevaMarca?.trim()) {
                                  agregarMarca(nuevaMarca.trim(), index)
                                }

                                return
                              }

                              actualizarVehiculoFormulario(index, 'marca', value)
                            }}
                            className="input-vam"
                          >
                            <option value="">Seleccionar marca</option>
                            {marcas.map((marca) => (
                              <option key={`${marca.id}-${marca.nombre}`} value={marca.nombre}>
                                {marca.nombre}
                              </option>
                            ))}
                        ...
                        <Campo label="Referencia parqueo">
                          <input
                            ...
                            className="input-vam"
                            placeholder="Opcional: P-01, pendiente..."
                          />
                        </Campo>

                        <Campo label="Número TAG / tarjeta">
                          <input
