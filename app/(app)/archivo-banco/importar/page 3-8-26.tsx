...
export default function IngresosBancariosUnificadoPage() {
    ...
    try {
      ...
      const [archivoExistenteResultado, pagosExistentesResultado, hashExistenteResultado] =
        await Promise.all([
          supabase
            .from("archivo_banco")
            .select("id, fecha_posteo, monto_transaccion, no_serial, descripcion, estado")
      ...
      if ((hashExistenteResultado.data || []).length > 0) {
      ...
      const resultado: FilaBanco[] = rows.map((row, index) => {
        ...
        if (!estadoEsError(estado) && clavesInternas.has(clave)) {
          ...
          observacion = `La transacción ya existe en archivo_banco con ID ${existente.id}.`;
        } else if (noSerial && referenciasPago.has(noSerial)) {
  ...
  function cambiarUnidad(index: number, unidadId: string) {
    setFilas((actuales) =>
      actuales.map((fila, filaIndex) => {
        if (filaIndex !== index) return fila;
        ...
        if (!unidadId) {
          return {
            ...
            estado: fila.archivo_banco_id ? "GUARDADO" : "REVISAR",
            observacion: "Seleccione manualmente la unidad correspondiente.",
          };
        }
        ...
        return {
          ...
          metodo_identificacion: "MANUAL",
          confianza_identificacion: 100,
          estado: fila.archivo_banco_id ? "GUARDADO" : "VALIDADO",
          observacion: "Unidad confirmada manualmente.",
        };
      }),
    );
  }
  ...
  async function guardarArchivoBanco() {
    ...
    const pendientesGuardar = filas.filter(
      (fila) =>
    ...
    if (pendientesGuardar.length === 0) {
      alert("No hay transacciones nuevas para guardar en archivo_banco.");
      return;
    }

    const total = pendientesGuardar.reduce(
      (suma, fila) => suma + Number(fila.monto_transaccion || 0),
      0,
    );

    const confirmar = confirm(
      `Se guardarán ${pendientesGuardar.length} transacción(es) bancarias por ${dinero(total)}.\n\n` +
        "Guardar el archivo NO crea pagos ni modifica cargos. ¿Desea continuar?",
    );
    ...
    try {
      const registros = pendientesGuardar.map((fila) => ({
      ...
      if (error) {
        throw new Error("Error guardando archivo bancario: " + error.message);
      }

      const idsPorClave = new Map<string, number>();
      ((data || []) as ArchivoBancoExistente[]).forEach((item) => {
        idsPorClave.set(claveTransaccion(item), item.id);
      });
      ...
      setFilas((actuales) =>
        actuales.map((fila) => {
          ...
          const id = idsPorClave.get(claveTransaccion(fila));
          if (!id) return fila;
      ...
      setMensaje(
        `Archivo guardado. Transacciones nuevas: ${pendientesGuardar.length}. Todavía no se han creado pagos.`,
      );
      log(`Archivo guardado en archivo_banco: ${pendientesGuardar.length} registro(s).`);
    } catch (error: any) {
      alert(error.message || "Error guardando el archivo.");
      log("ERROR: " + (error.message || "Error guardando el archivo."));
    }
  ...
  async function procesarPagosConfirmados() {
    ...
    const listos = filas.filter(
      (fila) =>
        ...
        fila.propietario_id &&
        !fila.procesado &&
        ["GUARDADO", "VALIDADO"].includes(fila.estado),
    );
  ...
  function exportarRevision() {
    ...
    XLSX.writeFile(libro, `revision-ingresos-${periodoArchivo}.xlsx`);
  }
                    ...
                    <tr key={`${fila.fila_origen}-${fila.no_serial}`} className="bg-white hover:bg-slate-50">
                      ...
                      <td className="min-w-[230px] px-3 py-3">
                        <select
                          ...
                          disabled={fila.procesado || estadoEsError(fila.estado) && !["ERROR_UNIDAD", "ERROR_PROPIETARIO"].includes(fila.estado)}
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-sm text-slate-600">
                      Guarda fecha, monto, serial y descripción en archivo_banco. No crea pagos.
                    </p>
                  </div>
                </div>
...
function EstadoBadge({ estado }: { estado: EstadoFila }) {
  ...
  const esListo = ["VALIDADO", "GUARDADO"].includes(estado);
