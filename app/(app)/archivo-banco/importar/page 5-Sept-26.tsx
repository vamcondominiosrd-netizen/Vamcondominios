...
function obtenerValor(row: Record<string, unknown>, opciones: string[]) {
  ...
  for (const opcion of opciones) {
    ...
    const encontrada = entradas.find(
      ([cabecera]) => normalizarTexto(cabecera) === buscada,
    );
...
function parsearFecha(valor: unknown) {
  ...
  if (latino) {
    return `${latino[3]}-${String(latino[2]).padStart(2, "0")}-${String(
      latino[1],
    ).padStart(2, "0")}`;
  }
...
function tieneCabeceraApartamento(row: Record<string, unknown>) {
  ...
  return CABECERAS_APARTAMENTO.some((cabecera) =>
    cabeceras.includes(normalizarTexto(cabecera)),
  );
}
...
function buscarUnidadPorCodigo(valor: string, unidades: Unidad[]) {
  ...
  const exacta = unidades.find(
    (unidad) => normalizarCodigo(unidad.codigo) === codigoBuscado,
  );
  ...
  const candidatas = unidades.filter(
    (unidad) =>
      obtenerApartamentoCorto(String(unidad.codigo || "")) === corta,
  );
...
function buscarUnidadEnDescripcion(
  ...
  const ordenadas = [...unidades].sort(
    (a, b) =>
      normalizarTexto(b.codigo || "").length -
      normalizarTexto(a.codigo || "").length,
  );
...
function calcularCoincidenciaAlias(
  ...
  if (descripcion.length >= 8 && descripcionAlias.includes(descripcion)) {
    return 95;
  }
...
function buscarMejorAlias(
  descripcion: string,
  aliasRows: AliasBanco[],
) {
  return (
    aliasRows
      .map((alias) => ({
        alias,
        puntos: calcularCoincidenciaAlias(descripcion, alias),
      }))
      .filter((resultado) => resultado.puntos >= 60)
      .sort((a, b) => {
        if (b.puntos !== a.puntos) return b.puntos - a.puntos;
        return (
          normalizarTexto(b.alias.descripcion_banco || "").length -
...
function registroEstaProcesado(estado: string | null | undefined) {
  ...
  return ["procesado", "pagado", "aplicado"].some((palabra) =>
...
function identificacionEsDiferente({
  ...
  return (
    ...
    normalizarCodigo(apartamentoActual) !==
      normalizarCodigo(apartamentoGuardado) ||
    normalizarTexto(propietarioActual) !==
      normalizarTexto(propietarioGuardado)
  );
}
...
function marcaTiempoArchivo() {
  ...
  const hora = `${String(ahora.getHours()).padStart(2, "0")}${String(
    ahora.getMinutes(),
  ).padStart(2, "0")}${String(ahora.getSeconds()).padStart(
    2,
    "0",
...
export default function IngresosBancariosUnificadoPage() {
  ...
  const [periodoArchivo, setPeriodoArchivo] = useState(
  ...
  const [modalidad, setModalidad] =
    useState<ModalidadArchivo | null>(null);
  ...
  async function cargarDatosIniciales(id: string) {
    ...
    const [unidadesResultado, cuentasResultado, aliasResultado] =
      await Promise.all([
        supabase
          .from("unidades")
          .select(
            "id, codigo, propietario_id, propietario_nombre",
          )
        ...
        supabase
          ...
          .select(
            "id, nombre_banco, numero_cuenta, fondo_tipo",
          )
          .eq("condominio_id", Number(id))
          .eq("activa", true)
        ...
        supabase
          ...
          .select(
            "id, unidad_id, no_apartamento, propietario, descripcion_banco",
          )
          .eq("condominio_id", Number(id))
          .eq("estado", "Activo")
    ...
    if (unidadesResultado.error) {
      alert(
        "Error cargando unidades: " +
          unidadesResultado.error.message,
      );
      return;
    }
    ...
    if (cuentasResultado.error) {
      alert(
        "Error cargando cuentas bancarias: " +
          cuentasResultado.error.message,
      );
      return;
    }
    ...
    if (aliasResultado.error) {
      alert(
        "Error cargando alias bancarios: " +
          aliasResultado.error.message,
      );
      return;
    }

    const cuentasData =
      (cuentasResultado.data || []) as CuentaBancaria[];
    ...
    if (!cuentaBancariaId && cuentasData.length > 0) {
      ...
      setCuentaBancariaId(
        String(ordinaria?.id || cuentasData[0].id),
      );
    }
  }
  ...
  async function seleccionarArchivo(file: File | null) {
    ...
    setArchivoHash(hash);
    log(
      `Archivo seleccionado: ${file.name}. Hash: ${hash.slice(
        0,
        16,
      )}...`,
    );
  ...
  async function leerYAnalizarArchivo() {
    ...
    if (!cuentaBancariaId) {
      alert(
        "Debe seleccionar la cuenta bancaria que recibió los ingresos.",
      );
      return;
    }
    ...
    try {
      ...
      const rows =
        XLSX.utils.sheet_to_json<Record<string, unknown>>(hoja, {
          defval: "",
          raw: true,
        });
      ...
      const modalidadDetectada: ModalidadArchivo =
        tieneCabeceraApartamento(rows[0])
          ? "IDENTIFICADO"
          : "SIN_IDENTIFICAR";
      ...
      const [
        archivoExistenteResultado,
        pagosExistentesResultado,
        hashExistenteResultado,
      ] = await Promise.all([
        supabase
          .from("archivo_banco")
          .select(
      ...
      if (archivoExistenteResultado.error) {
        throw new Error(
          "Error verificando transacciones importadas: " +
            archivoExistenteResultado.error.message,
        );
      }

      if (pagosExistentesResultado.error) {
        throw new Error(
          "Error verificando pagos existentes: " +
            pagosExistentesResultado.error.message,
        );
      }

      if (hashExistenteResultado.error) {
        throw new Error(
          "Error verificando el archivo por hash: " +
            hashExistenteResultado.error.message,
        );
      }

      const archivoYaImportado =
        (hashExistenteResultado.data || []).length > 0;
      ...
      const existentesPorClave = new Map<
        string,
        ArchivoBancoExistente
      >();
      const existentesAgrupadosPorSerial = new Map<
        string,
        ArchivoBancoExistente[]
      >();

      (
        (archivoExistenteResultado.data ||
          []) as ArchivoBancoExistente[]
      ).forEach((item) => {
        existentesPorClave.set(claveTransaccion(item), item);

        const serialNormalizado = normalizarTexto(
          item.no_serial || "",
        );

        if (serialNormalizado) {
          const grupo =
            existentesAgrupadosPorSerial.get(serialNormalizado) ||
            [];
          grupo.push(item);
          existentesAgrupadosPorSerial.set(
            serialNormalizado,
            grupo,
          );
        }
      ...
      existentesAgrupadosPorSerial.forEach((grupo, serial) => {
        if (grupo.length === 1) {
          existentesPorSerial.set(serial, grupo[0]);
        }
      });

      const referenciasPago = new Set(
        (
          (pagosExistentesResultado.data || []) as Array<{
      ...
      const resultado: FilaBanco[] = rows.map((row, index) => {
        ...
        let metodo: FilaBanco["metodo_identificacion"] =
          "NO_IDENTIFICADO";
        let confianza = 0;

        if (apartamentoOriginal) {
          unidad = buscarUnidadPorCodigo(
            apartamentoOriginal,
            unidades,
          );
        ...
        if (!unidad && descripcion) {
          unidad = buscarUnidadEnDescripcion(
            descripcion,
            unidades,
          );
        ...
        if (!unidad && descripcion) {
          const mejorAlias = buscarMejorAlias(
            descripcion,
            aliasBanco,
          );
          ...
          if (mejorAlias?.alias.unidad_id) {
            unidad =
              unidades.find(
                (item) =>
                  item.id === mejorAlias.alias.unidad_id,
              ) || null;
        ...
        const existente =
          ...
          (noSerial
            ? existentesPorSerial.get(
                normalizarTexto(noSerial),
              )
            : undefined);

        const unidadIdGuardada = existente?.unidad_id
          ? Number(existente.unidad_id)
          : null;

        const apartamentoGuardado = limpiarTexto(
          existente?.apartamento || "",
        );

        const propietarioGuardado = limpiarTexto(
          existente?.propietario || "",
        );

        const unidadGuardada = unidadIdGuardada
          ? unidades.find(
              (item) => item.id === unidadIdGuardada,
            ) || null
          : null;
        ...
        const pagoYaExiste = Boolean(
          noSerial &&
            referenciasPago.has(normalizarTexto(noSerial)),
        );

        const existenteProcesado = Boolean(
          existente &&
            (registroEstaProcesado(existente.estado) ||
              pagoYaExiste),
        );

        let estado: EstadoFila = unidad
          ? "VALIDADO"
          : "REVISAR";

        let observacion = unidad
          ? `Unidad identificada mediante ${metodo
              .toLowerCase()
              .replaceAll("_", " ")}.`
        ...
        if (!fecha) {
          estado = "ERROR_FECHA";
          observacion =
            "La fecha bancaria está vacía o no es válida.";
          ...
          observacion = `La unidad ${
            unidad.codigo || apartamentoOriginal
          } no tiene propietario asignado.`;
        }

        if (
          !estadoEsError(estado) &&
          clavesInternas.has(clave)
        ) {
          estado = "DUPLICADO_INTERNO";
          observacion =
            "Registro duplicado dentro del mismo archivo.";
          ...
          if (
            ...
            observacion =
              ...
              `${apartamentoGuardado || "Sin identificar"} → ${
                unidad.codigo || apartamentoOriginal
              }.`;
          ...
          observacion =
            "Ya existe un pago registrado con este número serial o referencia.";
        }
      ...
      const cambiosDetectados = resultado.filter(
        (fila) => fila.estado === "ACTUALIZAR",
      ).length;

      const existentesSinCambios = resultado.filter(
        (fila) => fila.estado === "SIN_CAMBIOS",
      ).length;

      const protegidos = resultado.filter(
        (fila) =>
          fila.estado === "PROTEGIDO_PROCESADO",
      ).length;
      ...
      log(
        `Paso 3 OK: ${resultado.length} registro(s) analizados.`,
      );
    } catch (error: any) {
      alert(
        error.message || "Error analizando el archivo.",
      );
      log(
        "ERROR: " +
          (error.message ||
          ...
          fila.archivo_banco_id
            ? identificacionEsDiferente({
                ...
                propietarioActual:
                  unidad.propietario_nombre || "",
                unidadIdGuardada:
                  fila.unidad_id_guardada,
                apartamentoGuardado:
                  fila.apartamento_guardado,
                propietarioGuardado:
                  fila.propietario_guardado,
              })
            : false;
  ...
  async function guardarArchivoBanco() {
    ...
    if (
      pendientesInsertar.length === 0 &&
      pendientesActualizar.length === 0
    ) {
      alert(
        "No hay transacciones nuevas ni cambios de identificación pendientes de guardar.",
      );
      return;
    }
    ...
    const totalNuevas = pendientesInsertar.reduce(
      (suma, fila) =>
        suma +
        Number(fila.monto_transaccion || 0),
      0,
    );

    const confirmar = confirm(
      `Transacciones nuevas: ${
        pendientesInsertar.length
      } por ${dinero(totalNuevas)}.\n` +
    ...
    try {
      const idsInsertadosPorClave = new Map<
        string,
        number
      >();

      if (pendientesInsertar.length > 0) {
        const registros = pendientesInsertar.map(
          (fila) => ({
            ...
            apartamento:
              fila.apartamento ||
              fila.apartamento_original ||
              null,
            propietario:
              fila.propietario || null,
            estado: "Revisar",
            periodo:
              fila.fecha_posteo.slice(0, 7),
            observacion:
        ...
        if (error) {
          throw new Error(
            "Error guardando transacciones nuevas: " +
              error.message,
          );
        }

        (
          (data || []) as ArchivoBancoExistente[]
        ).forEach((item) => {
          idsInsertadosPorClave.set(
            claveTransaccion(item),
            item.id,
          );
        });
      }
        ...
        actuales.map((fila) => {
          if (
            ...
            return {
              ...
              unidad_id_guardada:
                fila.unidad_id,
              apartamento_guardado:
                fila.apartamento,
              propietario_guardado:
                fila.propietario,
              requiere_actualizacion: false,
              estado: "GUARDADO",
              observacion:
                "Identificación actualizada correctamente y lista para registrar el pago.",
            };
          }

          if (fila.archivo_banco_id) {
            return fila;
          }

          const id =
            idsInsertadosPorClave.get(
              claveTransaccion(fila),
            );
      ...
      alert(
        error.message ||
          "Error guardando o actualizando el archivo.",
      );
      log(
        "ERROR: " +
          (error.message ||
  ...
  async function procesarPagosConfirmados() {
    ...
    if (listos.length === 0) {
      alert(
        "No hay transacciones guardadas e identificadas pendientes de procesar.",
      );
      return;
    }
    ...
    const confirmar = confirm(
      `Se crearán ${listos.length} pago(s) por ${dinero(
        total,
      )}.\n\n` +
    ...
    for (const filaLista of listos) {
      const index = filas.findIndex(
        (fila) =>
          fila.archivo_banco_id ===
            filaLista.archivo_banco_id &&
          fila.fila_origen ===
            filaLista.fila_origen,
      );

      try {
        log(
          `Procesando ${filaLista.apartamento} - ${dinero(
            filaLista.monto_transaccion,
          )} - archivo_banco ${
            filaLista.archivo_banco_id
          }`,
        );
        ...
        const pagoId = Number(
          (resultado as any)?.pago_id || 0,
        );

        const movimientoId = Number(
          (resultado as any)
            ?.movimiento_banco_id || 0,
        );

        if (!pagoId) {
          throw new Error(
            "La función no devolvió el ID del pago creado.",
          );
        }

        const asiento =
          await generarAsientoPagoMantenimiento({
            condominio_id:
              Number(condominioId),
            pago_id: pagoId,
            ...
            referencia:
              filaLista.no_serial ||
              `ARCHIVO_BANCO_${filaLista.archivo_banco_id}`,
    ...
    setMensaje(
      `Proceso terminado. Pagos registrados: ${procesados}. Errores: ${errores}.`,
    );
  ...
  function descargarPlantilla() {
    ...
    const instrucciones = [
      ...
      [
        "fecha",
        "Sí",
        "Fecha real de entrada en el banco.",
      ],
      [
        "monto",
        "Sí",
      ...
      [
        "no_serial",
        "Recomendado",
        "Referencia bancaria para evitar duplicados.",
      ],
      [
        "descripcion",
        "Recomendado",
    ...
    XLSX.writeFile(
      libro,
      "Plantilla_ingresos_bancarios_unificada.xlsx",
    );
  }
  ...
  function exportarRevision() {
    ...
    const datos = filas.map((fila) => ({
      ...
      Apartamento_original:
        fila.apartamento_original,
      Apartamento_identificado:
        fila.apartamento,
      Propietario: fila.propietario,
      Método_identificación:
        fila.metodo_identificacion,
      Confianza:
        fila.confianza_identificacion,
      Estado: fila.estado,
      Archivo_banco_id:
        fila.archivo_banco_id,
  ...
  const resumen = useMemo(() => {
    ...
    const procesados = filas.filter(
      (fila) => fila.procesado,
    );

    const errores = filas.filter((fila) =>
      estadoEsError(fila.estado),
    );
    ...
    return {
      ...
      identificados:
        identificados.length,
      montoIdentificado:
        identificados.reduce(
          (suma, fila) =>
            suma +
            Number(
              fila.monto_transaccion || 0,
  ...
  const cuentaSeleccionada = cuentas.find(
    (cuenta) =>
      cuenta.id === Number(cuentaBancariaId),
  );

  // Apartamentos que ya aparecen identificados en al menos una
  // transacción de la importación actual.
  const unidadesRepresentadasIds = useMemo(() => {
    const ids = new Set<number>();

    filas.forEach((fila) => {
      if (fila.unidad_id) {
        ids.add(Number(fila.unidad_id));
      }
    });
  ...
  // Apartamentos activos que todavía no aparecen asociados a
  // ninguna transacción identificada del archivo actual.
  // Se usan únicamente como candidatos de revisión.
  const unidadesNoPresentes = useMemo(
    () =>
      unidades.filter(
        (unidad) =>
          !unidadesRepresentadasIds.has(
            Number(unidad.id),
          ),
      ),
    [unidades, unidadesRepresentadasIds],
  );
  ...
  return (
    <PageContainer>
      ...
      <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 xl:grid-cols-5">
        <InfoBox
          label="Registros"
          value={String(resumen.registros)}
        />
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-amber-800">
                Leer y guardar el archivo no
                afecta pagos. La cuenta del
                propietario solo se modifica
                al presionar{" "}
                <strong>
      ...
      {filas.length > 0 && (
        <>
          {resumen.pendientes > 0 && (
            <SectionCard
              title="Apartamentos sin movimiento identificado en este archivo"
              subtitle="Úselos como referencia para validar las transacciones bancarias que todavía están pendientes de identificar."
              action={
                <div className="rounded-xl bg-amber-50 px-4 py-2 text-xs font-black text-amber-800">
                  Candidatos:{" "}
                  {
                    unidadesNoPresentes.length
                  }
                </div>
              }
            >
              <div className="mb-4 rounded-2xl border border-amber-200 bg-amber-50 p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sm font-black text-amber-900">
                      Apoyo para conciliación
                      manual
                    </p>
                    <p className="mt-1 text-sm text-amber-800">
                      Estos apartamentos no
                      tienen todavía una
                      transacción identificada
                      dentro de esta
                      importación. Compare el
                      nombre del propietario
                      con la descripción del
                      banco antes de seleccionar
                      una unidad. VAM no
                      realizará una asignación
                      automática basándose
                      solamente en esta lista.
                    </p>
                  </div>
                </div>
              </div>
              ...
              0 ? (
                <div className="rounded-2xl border border-blue-200 bg-blue-50 p-4 text-sm font-bold text-blue-800">
                  Todos los apartamentos ya
                  aparecen asociados a por lo
                  menos una transacción. Los
                  movimientos pendientes deben
                  revisarse como posibles pagos
                  adicionales, abonos,
                  duplicados u otros ingresos
                  antes de asignarlos.
                </div>
              ) : (
                <div className="max-h-72 overflow-auto rounded-2xl border bg-slate-50 p-4">
                  <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                    {unidadesNoPresentes.map(
                      (unidad) => (
                        <div
                          key={unidad.id}
            ...
            <div className="mb-5 grid grid-cols-1 gap-4 md:grid-cols-4">
              ...
              <div>
                ...
                >
                  ...
                  <option value="PROTEGIDO_PROCESADO">
                    Procesado protegido
                  </option>
                  <option value="ERROR_PROCESO">
                    Error de proceso
                  </option>
                  <option value="DUPLICADO_ARCHIVO">
                    Duplicado archivo
                  </option>
                  <option value="DUPLICADO_PAGO">
                    Duplicado pago
                  </option>
                </select>
              </div>
              ...
              <tbody className="divide-y divide-slate-200">
                {filasFiltradas.map(
                  (fila) => {
                    ...
                    return (
                      <tr
                        key={`${fila.fila_origen}-${fila.no_serial}`}
                        ...
                        <td className="min-w-[260px] px-3 py-3">
                          ...
                          >
                            ...
                            unidadesNoPresentes.length >
                              0 ? (
                              <>
                                <optgroup label="Primero revisar: no aparecen en el archivo">
                                  {unidadesNoPresentes.map(
                                    (
                                      unidad,
                                    ) => (
                                ...
                                {unidadesPresentes.length >
                                  0 && (
                                  <optgroup label="Otros apartamentos">
                                    {unidadesPresentes.map(
                                      (
                                        unidad,
                                      ) => (
                                        <option
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-sm text-slate-600">
                      Guarda
                      transacciones nuevas y
                      actualiza
                      identificaciones
                      corregidas en
                      archivo_banco. No crea
                      pagos.
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-sm text-emerald-800">
                      Crea el pago, aplica
                      cargos, genera saldo a
                      favor, registra el
                      ingreso bancario e
                      intenta crear el asiento
                      contable.
                    </p>
                  </div>
                </div>
      ...
      {bitacora.length > 0 && (
        <SectionCard
          title="Bitácora"
          subtitle="Seguimiento técnico de la importación actual."
        >
...
function EstadoBadge({
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-xs font-black ${clase}`}
    >
      {esProcesado || esListo ? (
