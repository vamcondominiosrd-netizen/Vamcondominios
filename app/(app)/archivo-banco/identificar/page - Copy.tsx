...
export default function IdentificarPagosPage() {
  ...
  const [alias, setAlias] = useState<AliasRow[]>([]);
  ...
  function compararDatos(bancoRows: BancoRow[], aliasRows: AliasRow[]) {
    const resultadoComparado = bancoRows.map((item) => {
      ...
      const encontrado = aliasRows.find((a) => {
        ...
        return descripcionBanco.includes(aliasTexto) || aliasTexto.includes(descripcionBanco);
      });
  ...
  const identificados = resultado.filter((r) => r.estado === "Identificado").length;
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-bold">Identificación Automática de Pagos</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="border rounded-lg p-4 bg-white">
          ...
          <h2 className="text-2xl font-bold text-green-700">{identificados}</h2>
        </div>
          ...
          <tbody>
            {resultado.map((r) => (
              <tr key={r.id}>
                ...
                <td className="p-2 border font-semibold">{r.apartamento_identificado}</td>

                <td
                  className={`p-2 border font-semibold ${
                    r.estado === "Identificado" ? "text-green-700" : "text-red-700"
                  }`}
                >
                  {r.estado}
                </td>
