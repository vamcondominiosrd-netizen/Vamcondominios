...
export default function CondominiosPage() {
  ...
  async function guardarCondominio(e: React.FormEvent) {
    ...
    if (errorBuscandoAdmin) {
      setGuardando(false);
      alert(
        "El condominio fue creado y asignado al selector, pero no se pudo validar el usuario administrador: " +
    ...
    if (adminExistente?.id) {
      const { error: errorReactivandoAdmin } = await supabase
        .from("usuarios_admin")
        .update({
      ...
      if (errorReactivandoAdmin) {
        setGuardando(false);
        alert(
          "El condominio fue creado y asignado al selector, pero no se pudo reactivar el usuario administrador: " +
            errorReactivandoAdmin.message,
        );
        return;
      }
    } else {
      ...
      if (errorAdmin) {
        setGuardando(false);
        alert(
          "El condominio fue creado y asignado al selector, pero no se pudo registrar en usuarios_admin: " +
            errorAdmin.message,
        );
        return;
      }
  ...
  async function cambiarEstado(id: number, nuevoEstado: string) {
    ...
    const { error } = await supabase
      .from("condominios")
      .update({ estado: nuevoEstado, activa: String(nuevoEstado).toLowerCase() === "activo" })
      .eq("id", id);
