...
export default function CondominiosPage() {
  ...
  function editarCondominio(c: Condominio) {
    ...
    setEstado((c.estado || "activo").toLowerCase());
  ...
  async function guardarCondominio(e: React.FormEvent) {
    ...
    const estadoNormalizado = estado.toLowerCase() === "activo" ? "Activo" : "Inactivo";
    ...
    if (editandoId) {
      ...
      // Mantiene actualizada la asignación del usuario en la tabla usada por el selector de condominios.
      await supabase
        ...
        .eq("condominio_id", editandoId);

      setGuardando(false);

      if (String(editandoId) === String(condominioActivoId)) {
        localStorage.setItem("condominio_nombre", nombreLimpio);
        ...
        setCondominioActivoNombre(nombreLimpio);
      }
    ...
    if (userError || !userData.user?.id) {
      setGuardando(false);
      alert("No se pudo identificar el usuario logueado. Inicie sesión nuevamente.");
      return;
    }
    ...
    if (error || !nuevoCondominio) {
      setGuardando(false);
      alert("Error guardando condominio: " + (error?.message || "No se obtuvo el ID creado."));
      return;
    }

    const { data: asignacionExistente, error: errorBuscandoAsignacion } = await supabase
      .from("usuarios_condominios")
      .select("id")
    ...
    if (errorBuscandoAsignacion) {
      setGuardando(false);
      alert(
        "El condominio fue creado, pero no se pudo validar la asignación del usuario: " +
    ...
    if (asignacionExistente?.id) {
      const { error: errorActualizandoAsignacion } = await supabase
        .from("usuarios_condominios")
        .update({
          nombre: nuevoCondominio.nombre,
          rol: "Administrador",
          estado: "Activo",
        })
        .eq("id", asignacionExistente.id);

      if (errorActualizandoAsignacion) {
        setGuardando(false);
        alert(
          "El condominio fue creado, pero no se pudo reactivar la asignación: " +
            errorActualizandoAsignacion.message
        );
        return;
      }
    } else {
      const { error: errorAsignacion } = await supabase.from("usuarios_condominios").insert({
      ...
      if (errorAsignacion) {
        setGuardando(false);
        alert(
          "El condominio fue creado, pero no se pudo asignar al usuario logueado: " +
    ...
    setGuardando(false);
    alert("Condominio registrado y asignado correctamente. Cierre sesión y vuelva a entrar para verlo en el selector.");
    limpiarFormulario();
    setMostrarFormulario(false);
    cargarCondominios(condominioActivoId);
  }
  ...
  async function cambiarEstado(id: number, nuevoEstado: string) {
    const estadoNormalizado = nuevoEstado.toLowerCase() === "activo" ? "Activo" : "Inactivo";
    const confirmar = confirm(`¿Desea cambiar el estado a ${estadoNormalizado}?`);
  ...
  const activos = condominios.filter((c) => (c.estado || "").toLowerCase() === "activo").length;
  const inactivos = condominios.filter((c) => (c.estado || "").toLowerCase() !== "activo").length;
