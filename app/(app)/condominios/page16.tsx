...
export default function CondominiosPage() {
  ...
  const [bitacoraProceso, setBitacoraProceso] = useState<BitacoraPaso[]>([]);
  ...
  function agregarBitacora(
    paso: string,
    estadoPaso: "OK" | "ERROR" | "INFO" = "INFO",
    detalle = "",
  ) {
    const item: BitacoraPaso = {
    ...
    console.log(`[Condominios] ${estadoPaso} - ${paso}`, detalle || "");
    setBitacoraProceso((actual) => [item, ...actual].slice(0, 30));
  }
  ...
  async function guardarCondominio(e: React.FormEvent) {
    ...
    setGuardando(true);
    setBitacoraProceso([]);
    agregarBitacora("Iniciando guardado del condominio", "INFO", nombre.trim());

    const empresaId = Number(localStorage.getItem("empresa_id") || "1");
    const sucursalId = Number(localStorage.getItem("sucursal_id") || "1");

    agregarBitacora(
      "Leyendo empresa y sucursal desde la sesión",
      "INFO",
    ...
    if (editandoId) {
      ...
      if (error) {
        agregarBitacora("Error actualizando condominio", "ERROR", error.message);
      ...
      agregarBitacora("Condominio actualizado correctamente", "OK", `id=${editandoId}`);
    ...
    agregarBitacora("Insertando registro en tabla condominios", "INFO");
    ...
    if (error) {
      setGuardando(false);
      agregarBitacora("Error guardando en condominios", "ERROR", error.message);
      alert("Error guardando condominio: " + error.message);
      return;
    }

    agregarBitacora("Condominio creado en tabla condominios", "OK", `id=${nuevoCondominio.id}, nombre=${nuevoCondominio.nombre}`);
    agregarBitacora("Identificando usuario logueado", "INFO");
    ...
    if (authError || !userId) {
      setGuardando(false);
      agregarBitacora("No se pudo identificar el usuario logueado", "ERROR", authError?.message || "userId vacío");
      alert(
    ...
    agregarBitacora("Validando asignación en usuarios_condominios", "INFO", `user_id=${userId}`);
    ...
    if (errorBuscandoAsignacion) {
      setGuardando(false);
      agregarBitacora("Error validando usuarios_condominios", "ERROR", errorBuscandoAsignacion.message);
      alert(
    ...
    if (asignacionExistente?.id) {
      ...
      if (errorReactivando) {
        setGuardando(false);
        agregarBitacora("Error reactivando usuarios_condominios", "ERROR", errorReactivando.message);
        alert(
      ...
      agregarBitacora("Asignación reactivada en usuarios_condominios", "OK", `id=${asignacionExistente.id}`);
    } else {
      agregarBitacora("Insertando asignación en usuarios_condominios", "INFO");
      ...
      if (errorAsignando) {
        setGuardando(false);
        agregarBitacora("Error insertando usuarios_condominios", "ERROR", errorAsignando.message);
        alert(
      ...
      agregarBitacora("Asignación creada en usuarios_condominios", "OK");
    }

    const usuarioNombre =
      localStorage.getItem("usuario_nombre") || "Administrador General";

    agregarBitacora("Validando acceso en usuarios_admin", "INFO");

    const { data: adminExistente, error: errorBuscandoAdmin } = await supabase
      .from("usuarios_admin")
      .select("id")
      .eq("user_id", userId)
      .eq("condominio_id", nuevoCondominio.id)
    ...
    if (errorBuscandoAdmin) {
      setGuardando(false);
      agregarBitacora("Error validando usuarios_admin", "ERROR", errorBuscandoAdmin.message);
      alert(
        "El condominio fue creado y asignado al selector, pero no se pudo validar usuarios_admin: " +
          errorBuscandoAdmin.message,
      );
      return;
    }

    if (adminExistente?.id) {
      const { error: errorActualizandoAdmin } = await supabase
        .from("usuarios_admin")
        .update({
      ...
      if (errorActualizandoAdmin) {
        setGuardando(false);
        agregarBitacora("Error actualizando usuarios_admin", "ERROR", errorActualizandoAdmin.message);
        alert(
          "El condominio fue creado y asignado al selector, pero no se pudo actualizar usuarios_admin: " +
            errorActualizandoAdmin.message,
        );
        return;
      }

      agregarBitacora("Usuario actualizado en usuarios_admin", "OK", `id=${adminExistente.id}`);
    } else {
      agregarBitacora("Insertando usuario en usuarios_admin", "INFO");
      const { error: errorInsertandoAdmin } = await supabase
        .from("usuarios_admin")
        .insert({
          user_id: userId,
          condominio_id: nuevoCondominio.id,
      ...
      if (errorInsertandoAdmin) {
        setGuardando(false);
        agregarBitacora("Error insertando usuarios_admin", "ERROR", errorInsertandoAdmin.message);
        alert(
          "El condominio fue creado y asignado al selector, pero no se pudo insertar en usuarios_admin: " +
            errorInsertandoAdmin.message,
        );
        return;
      }

      agregarBitacora("Usuario creado en usuarios_admin", "OK");
    }

    agregarBitacora("Proceso completado correctamente", "OK", `condominio_id=${nuevoCondominio.id}`);
    setGuardando(false);
    alert(
    ...
    <PageContainer>
      ...
      {bitacoraProceso.length > 0 && (
        <SectionCard
          ...
          subtitle="Seguimiento del proceso de creación/asignación del condominio."
        >
          <div className="max-h-80 space-y-2 overflow-auto rounded-xl border bg-slate-50 p-3">
            {bitacoraProceso.map((item, index) => (
              <div
              ...
              >
                ...
                <p className="mt-1 font-bold text-slate-800">{item.paso}</p>
                {item.detalle && (
