...
export default function CondominiosPage() {
  ...
  async function cargarCondominios(idActivo?: string) {
    ...
    const id = idActivo || localStorage.getItem("condominio_id") || condominioActivoId;
  ...
  async function obtenerContextoUsuario() {
    ...
    if (authError || !authData?.user?.id) {
      throw new Error("No se pudo identificar el usuario logueado. Inicie sesión nuevamente.");
    }
    ...
    const { data: perfil, error: perfilError } = await supabase
      .from("profiles")
      .select("id, empresa_id, client_id")
      .eq("id", userId)
      .maybeSingle();

    if (perfilError) {
      throw new Error("Error consultando el perfil del usuario: " + perfilError.message);
    }
    ...
    const clientId = Number(perfil?.client_id || perfil?.empresa_id || empresaId || 1);
  ...
  async function asignarCondominioAlUsuario(params: {
    userId: string;
    empresaId: number;
    condominioId: number;
  }) {
    const { userId, empresaId, condominioId } = params;
    ...
    if (errorConsulta) {
      throw new Error("Error verificando asignación del condominio: " + errorConsulta.message);
    }

    if (existente?.id) {
      if (!existente.activo) {
        const { error: errorActivando } = await supabase
          .from("usuarios_condominios")
          .update({ activo: true })
          .eq("id", existente.id);

        if (errorActivando) {
          throw new Error("El condominio fue creado, pero no se pudo reactivar la asignación: " + errorActivando.message);
        }
      }
    ...
    const { error: errorAsignando } = await supabase.from("usuarios_condominios").insert({
      user_id: userId,
      empresa_id: empresaId,
      condominio_id: condominioId,
      rol_condominio: "Administrador",
      activo: true,
    });

    if (errorAsignando) {
      throw new Error("El condominio fue creado, pero no se pudo asignar al usuario: " + errorAsignando.message);
    }
  }
  ...
  async function guardarCondominio(e: React.FormEvent) {
    ...
    try {
      ...
      const estadoNormalizado = estado === "inactivo" ? "inactivo" : "activo";
      ...
      const registro = {
        ...
        activa: estadoNormalizado === "activo",
      ...
      if (editandoId) {
        ...
        if (error) {
          throw new Error("Error actualizando condominio: " + error.message);
        }
      ...
      if (error) {
        throw new Error("Error guardando condominio: " + error.message);
      }

      if (!nuevoCondominio?.id) {
        throw new Error("El condominio fue creado, pero Supabase no devolvió el ID para asignarlo al usuario.");
      }

      await asignarCondominioAlUsuario({
        userId: contexto.userId,
        empresaId: contexto.empresaId,
        condominioId: Number(nuevoCondominio.id),
      });

      alert(
        "Condominio registrado correctamente y asignado al usuario. Cierre sesión y vuelva a entrar para verlo en la selección de condominios."
      );
      ...
      alert(err?.message || "Error guardando condominio.");
    } finally {
      setGuardando(false);
    }
  }
  ...
  async function cambiarEstado(id: number, nuevoEstado: string) {
    ...
    const estadoNormalizado = nuevoEstado === "inactivo" ? "inactivo" : "activo";

    const { error } = await supabase
      .from("condominios")
      .update({ estado: estadoNormalizado, activa: estadoNormalizado === "activo" })
      .eq("id", id);
  ...
  const activos = condominios.filter((c) => String(c.estado || "").toLowerCase() === "activo" || c.activa === true).length;
  const inactivos = condominios.length - activos;
    ...
    <PageContainer>
      ...
      >
        ...
        <div className="mt-4 overflow-x-auto rounded-2xl border">
          {loading ? (
            <div className="p-6 text-sm text-slate-500">Cargando condominio...</div>
          ) : (
                      ...
                      <td className="px-4 py-3 text-center">
                        <span
                          className={`rounded-full px-3 py-1 text-xs font-bold ${
                            String(c.estado || "").toLowerCase() === "activo" || c.activa === true
