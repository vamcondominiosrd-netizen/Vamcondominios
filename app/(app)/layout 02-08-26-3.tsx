...
const submenuResidencial: MenuItem[] = [
  ...
  { href: "/consulta-estado/configuracion-cargos", label: "Cuotas", icon: WalletCards },
...
const submenuConfiguracion: MenuItem[] = [
  ...
  { href: "/configuracion/modulos", label: "Módulos habilitados", icon: Package },
  { href: "/configuracion/roles", label: "Roles y permisos", icon: ShieldCheck },
];
...
function esRutaResidencial(path: string) {
  ...
  function esRutaConfiguracion(path: string) {
    return path === "/configuracion" || path.startsWith("/configuracion/");
  }

  function esRutaCatalogos(path: string) {
  ...
  function estaActivo(href: string) {
    ...
    if (href === "/catalogos") return esRutaCatalogos(pathname);
    if (href === "/configuracion") return esRutaConfiguracion(pathname);
  ...
  const centroActivo = useMemo(() => {
    ...
    if (esRutaConfiguracion(pathname)) {
      return { color: "slate", submenu: submenuConfiguracion };
    }
  ...
  function clasesCentro(activo: boolean) {
    ...
    if (activo) {
      ...
      if (centroActivo.color === "slate") return "bg-slate-700 text-white shadow-sm";
