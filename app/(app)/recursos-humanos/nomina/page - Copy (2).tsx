...
export default function NominaPage() {
  ...
  async function generarGastoNomina(nominaId: number) {
    ...
    if (!nominaActual) {
      throw new Error("No se encontró la nómina para generar el gasto.");
    }

    if (nominaActual.gasto_generado && nominaActual.gasto_id) {
      return;
    }

    const fechaGasto =
      nominaActual.fecha_pago || new Date().toISOString().slice(0, 10);

    const descripcion = `Nómina ${nominaActual.tipo_nomina || "Regular"} - ${
      nominaActual.nombre_empleado || "Empleado"
    ...
    const detalle = [
      ...
      `Neto pagado: RD$${moneda(Number(nominaActual.neto_pagar || 0))}`,
      "Generado automáticamente desde Recursos Humanos / Nómina.",
    ].join("\n");
      ...
      .insert([
        {
          ...
          proveedor: nominaActual.nombre_empleado || "Empleado",
    ...
    if (errorGasto) {
      throw new Error(errorGasto.message);
    }

    const { error: errorActualizarNomina } = await supabase
      .from("rh_nomina")
      .update({
        gasto_id: Number(gastoCreado.id),
        gasto_generado: true,
      })
      .eq("id", nominaId)
      .eq("condominio_id", Number(condominioId));
  ...
  async function guardarNomina(e: React.FormEvent) {
    ...
    try {
      if (editandoId) {
        ...
        if (estado === "Pagada") {
          await generarGastoNomina(editandoId);
        }

        setGuardando(false);
        alert(
          estado === "Pagada"
            ? "Nómina modificada y gasto generado correctamente."
      ...
      if (estado === "Pagada") {
        await generarGastoNomina(nuevaNominaId);
      }

      setGuardando(false);
      alert(
        estado === "Pagada"
          ? "Nómina registrada y gasto generado correctamente."
  ...
  async function cambiarEstado(nomina: Nomina, nuevoEstado: string) {
    ...
    if (nuevoEstado === "Pagada") {
      try {
        await generarGastoNomina(nomina.id);
      } catch (error: any) {
        alert(
          "El estado fue actualizado, pero ocurrió un error generando el gasto: " +
            error.message
        );
        cargarNominas(condominioId, filtroPeriodo);
        return;
    ...
    alert(
      nuevoEstado === "Pagada"
        ? "Estado actualizado y gasto generado correctamente."
