...
export default function GasRecepcionPage() {
  ...
  async function generarSolicitudPago(r: GasRecepcion) {
    ...
    const concepto = `Pago de gas según factura ${r.no_factura}`;

    const detalle = `
Pago de gas registrado desde el módulo de Gas.

Datos de factura:
Proveedor: ${proveedorNombre}
Factura No.: ${r.no_factura || "-"}
NCF: ${r.ncf || "-"}
Fecha factura: ${r.fecha_factura || "-"}
Monto: RD$ ${dinero(r.monto_factura)}
ITBIS: RD$ ${dinero(r.itbis_factura)}
Total: RD$ ${dinero(r.total_factura)}

Datos de recepción:
Conduce No.: ${r.no_conduce || "-"}
Fecha conduce: ${r.fecha_conduce || "-"}
Tanque / ubicación del condominio: ${tanqueNombre}
Cantidad recibida: ${dinero(r.cantidad)} ${unidadNombre}
Precio estimado unitario: RD$ ${dinero(r.precio_unitario)}
Total estimado recepción: RD$ ${dinero(r.total)}
Responsable recepción: ${r.responsable || "-"}
