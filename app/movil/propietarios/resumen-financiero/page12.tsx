...
export default function TransparenciaFinancieraPage(){
 ...
 useEffect(()=>{if(propietario?.condominio_id)void cargarPeriodo(propietario,periodo)},[periodo,propietario?.condominio_id]);
 ...
 async function inicializar(){
  ...
  try{
   ...
   setCuenta((cuentaData as CuentaBancaria)||null);const cierresCerrados=((cierresData||[]) as CierreBancario[]).filter(x=>esPeriodoCerrado(x.estado));setCierres(cierresCerrados);if(cierresCerrados.length>0){setPeriodo(cierresCerrados[0].periodo);await cargarPeriodo(s,cierresCerrados[0].periodo);}else{setCierre(null);setGastos([]);}
 ...
 return <main className="min-h-dvh bg-slate-100 pb-8">
  ...
  <div className="mx-auto max-w-lg space-y-4 px-4 pt-4">
   <section className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm"><div className="flex items-center justify-between gap-3"><label className="text-[11px] font-extrabold uppercase tracking-wide text-slate-500">Periodo cerrado</label>{cierre&&<span className="rounded-full border border-emerald-200 bg-emerald-50 px-2.5 py-1 text-[10px] font-black text-emerald-700">CERRADO</span>}</div>{periodos.length>0?<select value={periodo} onChange={e=>setPeriodo(e.target.value)} className="mt-2 h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm font-bold text-slate-700">{periodos.map(p=><option key={p} value={p}>{periodName(p)}</option>)}</select>:<div className="mt-2 rounded-xl bg-slate-100 px-3 py-4 text-center text-xs text-slate-500">No hay estados financieros cerrados disponibles.</div>}<p className="mt-2 flex items-center gap-1 text-[10px] text-slate-400"><Landmark size={12}/>{cuenta?`${cuenta.nombre_banco||"Banco"} · ${cuenta.numero_cuenta||"Sin número"}`:"Cuenta bancaria no identificada"}</p>{cierre&&<p className="mt-2 rounded-xl border border-blue-100 bg-blue-50 px-3 py-2 text-[10px] leading-4 text-blue-800">La información corresponde a un periodo financiero cerrado y refleja los movimientos registrados por la administración.</p>}</section>
