...
export default function LoginMovilVAMPage() {
  ...
  function guardarSesionPropietario(
    ...
    if (expiraEn) {
      localStorage.setItem("propietario_token_expira", expiraEn);
    }
  ...
  async function completarIngresoPropietario(
    propiedad: PropietarioSesion,
    token: string,
    expiraEn?: string
  ) {
    ...
    const sesion = guardarSesionPropietario(propiedad, token, expiraEn);
  ...
  async function procesarRespuestaPropietario(
    data: unknown,
    mensajePredeterminado: string
  ) {
    const respuesta = extraerRespuesta<RespuestaPropietario>(data);
    const fueCorrecto = respuesta.ok ?? respuesta.success ?? false;
    const propiedades = Array.isArray(respuesta.propiedades)
      ? respuesta.propiedades
      : [];
    const token = String(respuesta.token || "");

    if (!fueCorrecto || !token || propiedades.length === 0) {
      mostrarError(
        respuesta.mensaje ||
          respuesta.message ||
    ...
    if (propiedades.length > 1) {
      ...
      if (respuesta.expira_en) {
        localStorage.setItem(
          ...
          String(respuesta.expira_en)
        );
      }

      setPropiedadesDisponibles(propiedades);
      setMostrarSelectorPropiedad(true);
      mostrarExito("Seleccione la propiedad que desea consultar.");
      return;
    }

    await completarIngresoPropietario(
      propiedades[0],
      token,
      respuesta.expira_en
    );
  ...
  async function entrarPropietario(event?: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      await procesarRespuestaPropietario(
        data,
  ...
  async function seleccionarPropiedad(propiedad: PropietarioSesion) {
    if (!tokenPropietario) {
      mostrarError("La sesión del propietario no está disponible.");
      setMostrarSelectorPropiedad(false);
      return;
    }
  ...
  function cancelarSeleccionPropiedad() {
    limpiarSesionesLocales();
    setMostrarSelectorPropiedad(false);
    setPropiedadesDisponibles([]);
    setTokenPropietario("");
                        ...
                        <div className="flex items-start justify-between gap-3">
                          <div>
                            ...
                            <p className="mt-1 text-xs leading-5 text-slate-500">
                              Use su cédula y contraseña. Si tiene varias propiedades,
                              podrá elegir cuál desea consultar.
                            </p>
                          </div>
      ...
      {mostrarSelectorPropiedad && propiedadesDisponibles.length > 0 && (
        <div className="fixed inset-0 z-[190] flex items-center justify-center bg-slate-950/80 p-3 backdrop-blur-sm">
          <section
            role="dialog"
            aria-modal="true"
            aria-labelledby="titulo-seleccionar-propiedad"
            className="max-h-[92dvh] w-full max-w-md overflow-y-auto rounded-[1.75rem] bg-white shadow-2xl"
          >
            <header className="bg-gradient-to-r from-blue-800 to-slate-950 px-5 py-5 text-white">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <h2
                    id="titulo-seleccionar-propiedad"
                    className="mt-1 text-xl font-black"
                  >
                    Seleccione dónde desea entrar
                  </h2>
                </div>
              </div>
            </header>
            ...
            <div className="space-y-3 p-5">
              {propiedadesDisponibles.map((propiedad) => (
                <button
                  key={`${propiedad.condominio_id}-${propiedad.unidad_id}`}
                  type="button"
                  onClick={() => void seleccionarPropiedad(propiedad)}
                  disabled={loading}
