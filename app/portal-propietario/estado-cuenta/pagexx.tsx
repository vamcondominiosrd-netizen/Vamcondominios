...
export default function EstadoCuentaPage() {
  ...
  async function consultarEstadoCuenta(
    ...
    const { data: propietarioData, error } =
      await supabase
        .from("propietarios_apartamentos")
        .select(
          "id, condominio, no_apartamento, nombre_propietario, cedula"
        )
        .eq("condominio", condominio)
        .ilike("no_apartamento", apartamento)
        .eq("cedula", cedula)
        .maybeSingle();
    ...
    const { data: cargosData, error: errorCargos } =
      await supabase
  ...
  const balancePendiente = cargos
    ...
    .reduce(
      (sum, c) => sum + Number(c.total_cargo || 0),
      0
    );
