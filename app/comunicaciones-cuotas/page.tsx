...
type ProgramarCanalesResponse = {
  ok: boolean;
  mensaje?: string;
  email_programados?: number;
  whatsapp_programados?: number;
  sin_email?: number;
  sin_whatsapp?: number;
};

type ProcesarEnviosResponse = {
  ok: boolean;
  mensaje?: string;
  procesados?: number;
  enviados?: number;
  fallidos?: number;
  pendientes_configuracion?: number;
};
...
export default function ComunicacionesCuotaPage() {
  ...
  async function cargarVistaPrevia(id = condominioId, periodoValor = periodo) {
    ...
    try {
      ...
      if (
        unidadSeleccionada &&
        !disponibles.some(
          (item) => String(item.unidad_id) === String(unidadSeleccionada),
        )
      ) {
  ...
  async function programarYProcesarCanales(
    unidadId: number | null,
  ): Promise<{ ok: boolean; texto: string }> {
    if (!enviarEmail && !enviarWhatsapp) {
      return {
        ok: true,
        texto: "Disponible en VAM App.",
      };
    }

    const { data: programacionData, error: programacionError } =
      await supabase.rpc("programar_envios_recordatorios_cuota", {
        p_condominio_id: Number(condominioId),
        p_periodo: periodo,
        p_unidad_id: unidadId,
        p_email: enviarEmail,
        p_whatsapp: enviarWhatsapp,
      });
    ...
    if (programacionError) {
      return {
        ok: false,
        texto:
          "La comunicación quedó disponible en VAM, pero no fue posible programar los canales externos: " +
          programacionError.message,
      };
    }

    const programacion =
      (programacionData || {}) as ProgramarCanalesResponse;
    ...
    const accessToken = sesionData.session?.access_token;

    if (!accessToken) {
      return {
        ok: false,
        texto:
          "Los canales externos quedaron programados, pero no se pudieron procesar porque la sesión administrativa no está disponible.",
      };
    }

    try {
      const respuesta = await fetch("/api/comunicaciones/procesar-envios", {
        method: "POST",
        headers: {
      ...
      const resultado =
        (await respuesta.json().catch(() => ({}))) as ProcesarEnviosResponse;

      const sinEmail = Number(programacion.sin_email || 0);
      const sinWhatsApp = Number(programacion.sin_whatsapp || 0);
      const enviados = Number(resultado.enviados || 0);
      const fallidos = Number(resultado.fallidos || 0);
      const pendientesConfig = Number(
        resultado.pendientes_configuracion || 0,
      );

      const partes: string[] = ["Disponible en VAM App."];

      if (enviarEmail) {
        partes.push(
          `Email programados: ${Number(
            programacion.email_programados || 0,
          )}${sinEmail > 0 ? ` · sin correo: ${sinEmail}` : ""}.`,
        );
      }

      if (enviarWhatsapp) {
        partes.push(
          `WhatsApp programados: ${Number(
            programacion.whatsapp_programados || 0,
          )}${sinWhatsApp > 0 ? ` · sin teléfono: ${sinWhatsApp}` : ""}.`,
        );
      }
      ...
      if (!respuesta.ok || !resultado.ok) {
        partes.push(
          resultado.mensaje ||
            "Los envíos externos quedaron pendientes de procesamiento.",
        );
        return { ok: false, texto: partes.join(" ") };
      }

      if (enviados > 0) {
        partes.push(`Envíos externos realizados: ${enviados}.`);
      }

      if (fallidos > 0) {
        partes.push(`Fallidos: ${fallidos}.`);
      }

      if (pendientesConfig > 0) {
        partes.push(
          `Pendientes por configuración del proveedor: ${pendientesConfig}.`,
        );
      }
      ...
      return {
        ok: false,
        texto:
          "Los canales externos quedaron programados, pero el procesador de envíos no respondió: " +
  ...
  async function generarComunicacionIndividual() {
    ...
    try {
      ...
      if (respuesta.generada === false) {
        ...
        const canales = await programarYProcesarCanales(
          Number(seleccionado.unidad_id),
        );

        const encabezado = `Comunicación generada correctamente para ${
          respuesta.propietario || seleccionado.propietario
        } — Apto. ${respuesta.unidad || seleccionado.unidad}.`;

        if (canales.ok) {
          setExito(`${encabezado} ${canales.texto}`);
        } else {
          setExito(encabezado);
          setMensaje(canales.texto);
        }
      }
  ...
  async function generarComunicaciones() {
    ...
    const confirmado = window.confirm(
      `Esta opción generará ${cantidad} comunicación${
        cantidad === 1 ? "" : "es"
    ...
    try {
      ...
      const canales = await programarYProcesarCanales(null);

      const encabezado = `Proceso completado: ${Number(
  ...
  const candidatosIndividual = useMemo(
    () => items.filter((item) => !item.ya_generada),
    [items],
  );

  const propietarioIndividual = useMemo(
    () =>
      items.find(
        (item) => String(item.unidad_id) === String(unidadSeleccionada),
      ) || null,
        ...
        <section className="rounded-[1.6rem] border border-slate-200 bg-white p-5 shadow-sm">
          <div>
            ...
            <p className="mt-1 text-xs text-slate-500">
              La comunicación siempre queda disponible dentro de VAM. Seleccione
              los canales externos adicionales que desea utilizar.
            </p>
          </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <span className="rounded-full bg-blue-700 px-2.5 py-1 text-[10px] font-extrabold uppercase tracking-wide text-white">
                      Siempre activo
                    </span>
                  </div>
                  <p className="mt-1 text-xs leading-5 text-slate-600">
                    Alerta en el portal del propietario y conservación en el
            ...
            >
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-1 text-xs leading-5 text-slate-600">
                    Envía el recordatorio al correo registrado del propietario.
                  </p>
                </div>
              </div>
            </label>
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        setEnviarWhatsapp(event.target.checked)
                      }
                      className="h-4 w-4 accent-green-600"
                    />
                  </div>
                  <p className="mt-1 text-xs leading-5 text-slate-600">
                    Envía una plantilla aprobada al número registrado del
                    propietario.
                  </p>
                </div>
              </div>
          ...
          {(enviarEmail || enviarWhatsapp) && (
            <div className="mt-4 rounded-2xl bg-amber-50 px-4 py-3 text-xs leading-5 text-amber-800">
              Los canales externos requieren que el propietario tenga sus datos
              de contacto registrados y que el proveedor correspondiente esté
              configurado en el servidor. Si un envío externo falla, la
              comunicación seguirá disponible en VAM.
            </div>
          )}
        </section>
            ...
            <div className="min-w-0 flex-1">
              <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <p className="mt-1 text-xs text-slate-500">
                    Seleccione un propietario o apartamento para generar una comunicación individual.
                  </p>
                </div>
              ...
              <div className="mt-4 grid gap-3 lg:grid-cols-[1fr_auto] lg:items-end">
                ...
                <button
                  ...
                  disabled={
                    ...
                    propietarioIndividual.ya_generada ||
                    cargando ||
                    generando ||
                    generandoIndividual
                  }
                ...
                <div className="mt-4 grid gap-3 rounded-2xl bg-violet-50/70 p-4 sm:grid-cols-4">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-sm font-bold text-slate-800">
                      {propietarioIndividual.propietario}
                    </p>
                  </div>
                  <div>
                    <p className="text-[10px] font-bold uppercase tracking-wide text-slate-400">
                    ...
                    <p className="mt-1 text-sm font-bold text-slate-800">
                      {fechaCorta(propietarioIndividual.fecha_vencimiento)}
                    </p>
                  </div>
                  <div>
                    <p className="text-[10px] font-bold uppercase tracking-wide text-slate-400">
                    ...
                    <p className="mt-1 text-sm font-black text-red-600">
                      {moneda(propietarioIndividual.balance)}
                    </p>
                  </div>
                </div>
              )}
        ...
        <section className="overflow-hidden rounded-[1.6rem] border border-slate-200 bg-white shadow-sm">
          <div className="flex flex-col gap-3 border-b border-slate-100 p-5 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-xs text-slate-500">
                Revise los destinatarios antes de generar las comunicaciones para todo el condominio.
              </p>
            </div>
            ...
            >
              ...
              {generando
                ? "Generando..."
                : "Generar comunicaciones del condominio"}
            </button>
          </div>
