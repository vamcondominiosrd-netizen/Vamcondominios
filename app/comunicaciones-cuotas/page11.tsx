...
export default function ComunicacionesCuotaPage() {
  ...
  async function cargarVistaPrevia(id = condominioId, periodoValor = periodo) {
    ...
    try {
      ...
      if (!respuesta.ok) {
        setPreview(null);
        setMensaje(respuesta.mensaje || "No fue posible preparar la vista previa.");
        return;
      }
  ...
  async function generarComunicaciones() {
    ...
    const confirmado = window.confirm(
      `Se generarán ${cantidad} comunicación${
        cantidad === 1 ? "" : "es"
      } de recordatorio para ${preview?.periodo_texto || periodo}.\n\n¿Desea continuar?`,
    );
    ...
    try {
      ...
      if (!respuesta.ok) {
        setMensaje(respuesta.mensaje || "No fue posible generar las comunicaciones.");
        return;
      }

      setExito(
        `Proceso completado: ${Number(
          ...
          <div className="flex flex-col gap-3 border-b border-slate-100 p-5 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-xs text-slate-500">
                Revise los destinatarios antes de generar las comunicaciones.
              </p>
            </div>
            ...
            >
              ...
              {generando ? "Generando..." : "Generar comunicaciones"}
            </button>
          </div>
