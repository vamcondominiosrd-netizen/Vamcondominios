...
export default function MobilePagosIdentificadosPage() {
  ...
  const montoTotal = pagosFiltrados.reduce(
    (total, item) =>
      total + Number(item.monto_transaccion || item.monto || 0),
    0
  );

  return (
    <div className="min-h-screen bg-slate-50 pb-24">
      <div className="bg-green-700 text-white rounded-b-3xl p-5 shadow-md">
      ...
      <div className="p-4 space-y-4">
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-white border rounded-2xl p-4 shadow-sm">
            <p className="text-xs text-slate-500">Pagos visibles</p>
            <h2 className="text-2xl font-black">{totalPagos}</h2>
          </div>

          <div className="bg-white border rounded-2xl p-4 shadow-sm">
            <p className="text-xs text-slate-500">Monto visible</p>
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {pagosFiltrados.map((item) => {
              const apartamento =
                item.no_apartamento || item.apartamento || "-";

              const monto = Number(
                item.monto_transaccion || item.monto || 0
              );
