...
export default function LoginPage() {
  ...
  async function confirmarSesionActiva(uidEsperado: string) {
    const {
      data: { session },
      error,
    ...
    if (error) {
      throw new Error(`No se pudo confirmar la sesión: ${error.message}`);
    }

    if (!session || session.user.id !== uidEsperado) {
      throw new Error(
        "La sesión de Supabase no quedó disponible. Inicie sesión nuevamente."
      );
    }
  ...
  async function navegarAlDashboard(uid: string) {
    await confirmarSesionActiva(uid);

    // Navegación completa para que AuthGuard y el layout lean la sesión
    // persistida y las claves de contexto desde el mismo origen.
  ...
  async function iniciarSesion(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (error || !data.user) {
        mostrarError(error?.message || "Usuario o clave incorrecta.");
        return;
      }
      ...
      if (empresaError) {
        mostrarError(
          `No se pudo consultar la empresa asignada: ${empresaError.message}`
        );
        return;
      }
      ...
      if (!empresaData) {
        ...
        mostrarError(
          "Este usuario no tiene una empresa activa asignada en VAM Enterprise."
        );
        return;
      }
      ...
      const usuarioEmpresaActualId = Number((empresaData as any).id);
      ...
      if (!usuarioEmpresaActualId || !empresaActualId) {
        mostrarError(
          "La asignación empresarial del usuario está incompleta."
        );
        return;
      }

      setUsuarioEmpresaId(usuarioEmpresaActualId);
      ...
      // Se consulta primero la asignación y luego el catálogo de condominios.
      // Esto evita que una relación embebida o una política RLS oculte el error real.
      const { data: accesosData, error: accesosError } = await supabase
        .from("usuarios_condominios")
        .select("id, condominio_id, rol_condominio")
        .eq("user_id", uid)
      ...
      if (accesosError) {
        mostrarError(
          `No se pudieron consultar los accesos a condominios: ${accesosError.message}`
        );
        return;
      }

      const accesos = (accesosData || []).filter(
        (item: any) => Number(item.condominio_id) > 0
      );

      if (accesos.length === 0) {
        await supabase.auth.signOut();
        mostrarError("Este usuario no tiene condominios activos asignados.");
        return;
      }

      const idsCondominios = Array.from(
        new Set(accesos.map((item: any) => Number(item.condominio_id)))
      );
      ...
      if (condominiosError) {
        mostrarError(
          `No se pudo cargar el catálogo de condominios: ${condominiosError.message}`
        );
        return;
      }

      const detallePorId = new Map<number, any>(
        (condominiosData || []).map((item: any) => [Number(item.id), item])
      );

      const lista: CondominioAsignado[] = accesos
        .map((acceso: any) => {
          const detalle = detallePorId.get(Number(acceso.condominio_id));

          return {
            id: Number(acceso.condominio_id),
            nombre: detalle?.nombre || "",
            logo_url: detalle?.logo_url || null,
            acceso_id: Number(acceso.id),
            rol_condominio: acceso.rol_condominio || rolGlobalActual,
          };
        })
        .filter(
          (condominio) =>
            condominio.id > 0 &&
            condominio.acceso_id > 0 &&
            Boolean(condominio.nombre)
        );

      if (lista.length === 0) {
        mostrarError(
          "Las asignaciones existen, pero no fue posible relacionarlas con el catálogo de condominios."
        );
        return;
      }
      ...
      if (lista.length === 1) {
        const modulos = await cargarModulosCondominio(lista[0].id);

        guardarSesion({
          uid,
          usuarioEmpresaId: usuarioEmpresaActualId,
          empresaActualId,
          empresaNombreActual,
          usuarioNombreActual,
          rolGlobalActual,
  ...
  async function cargarModulosCondominio(
    condominioActualId: number
  ): Promise<string[]> {
    const { data, error } = await supabase.rpc(
      "obtener_modulos_usuario_condominio",
      {
        p_condominio_id: condominioActualId,
      }
    );

    if (error) {
      throw new Error(
        `No fue posible cargar los módulos habilitados: ${error.message}`
      );
    }
  ...
  function guardarSesion({
    ...
    localStorage.setItem(
      "usuario_empresa_id",
      String(usuarioEmpresaActualId)
    );
    localStorage.setItem(
      "usuario_condominio_id",
      String(condominio.acceso_id)
    );
    ...
    localStorage.setItem("rol_condominio", condominio.rol_condominio);
  ...
  async function entrarConCondominio() {
    ...
    try {
      await confirmarSesionActiva(userId);

      const { data: accesoActual, error: accesoError } = await supabase
        .from("usuarios_condominios")
        .select("id, rol_condominio, activo")
        .eq("id", condominio.acceso_id)
        .eq("user_id", userId)
      ...
      if (accesoError) {
        throw new Error(
          `No se pudo revalidar el acceso seleccionado: ${accesoError.message}`
        );
      }

      if (!accesoActual) {
        throw new Error(
          "El acceso a este condominio ya no está activo o no pertenece al usuario."
        );
      }

      const permisos = await cargarPermisosUsuario(userId, empresaId);
      const modulos = await cargarModulosCondominio(condominio.id);
      ...
      guardarSesion({
        ...
        condominio: {
          ...condominio,
          acceso_id: Number(accesoActual.id),
          rol_condominio:
            accesoActual.rol_condominio || condominio.rol_condominio,
        },
        permisos,
        modulos,
      });

      await navegarAlDashboard(userId);
    } catch (error) {
      console.error("Error entrando al condominio:", error);
      mostrarError(
        error instanceof Error
          ? error.message
          : "No fue posible entrar al condominio seleccionado."
      );
    } finally {
      setLoading(false);
    }
